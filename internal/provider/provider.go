...
// well-formed: pass through without allocating
...
// prompt tokens not cached, including CacheWriteTokens
// subset of CacheMissTokens used to create provider cache entries
// cache-write charge expressed in ordinary input-token equivalents
// subset of CompletionTokens spent on chain-of-thought
// "stop", "tool_calls", "length", "content_filter", "repetition_truncation", …
...
// ChunkReasoning: opaque proof for the reasoning (Anthropic thinking signature), when issued
...
// ChunkReasoning: provider-issued reasoning item id
// ChunkReasoning: final reasoning item status ("completed")
// ChunkToolCallStart (ID+Name only), ChunkToolCallArgsDelta (ID+Name), ChunkToolCall (complete)
// ChunkToolCallArgsDelta: cumulative argument characters received for this call
// ChunkResponsesItem: opaque validated Responses API output item
// ChunkUsage
// ChunkError
...
// one of the StreamInterrupt* constants; may be empty for older callers
