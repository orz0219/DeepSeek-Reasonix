...
import { Activity, Command, Download, Search, Server, SquarePen, PanelLeft, PanelRight, FileDown, FileImage, FileText, FileJson, GitBranch, MessageSquare, Settings as SettingsIcon, Pencil, RotateCw, Trash2, AlarmClock, BarChart3, Brain, Cpu, Palette, Puzzle, TerminalSquare } from "lucide-react";
...
import { type BotRuntimeStatusView, type ActiveWorkView, type BackgroundRuntimeView, type CollaborationMode, type ComposerInsertRequest, type DesktopStartupSettingsView, type Mode, modeHasPlan, type RewindResultView, type RemoteHostView, type SessionMeta, type SettingsView, type TabMeta, type TokenMode, type ToolApprovalMode, type WorkspaceConflictView } from "./lib/types";
...
import { applyTheme, clearLegacyThemePreference, getTheme, getThemeStyle, isThemeStyle, normalizeThemePreference, normalizeThemeStyleForTheme, readLegacyThemePreference } from "./lib/theme";
...
import { SidebarImConnection, SidebarImTopicSource, sidebarImConnectionsFromBot, taskSessionIDFromPath, sidebarImSessionTarget, isChannelSession, sidebarImTopicSourcesFromBot, sidebarImScopeLabel, SidebarImConnectionDetail } from "./app_sidebar";
...
export default function App() {
    const { state, liveStore, activeTabId, sendToTab, recoverDeliveryToTab, waiveDeliveryToTab, runShellForTab, steerForTab, notice, cancel, approve, resolvePlanDecision, resolveRecovery, answerQuestion, setControllerMode, dismissExtensionForm, drainExtensionNotifications, setCollaborationMode: setControllerCollaborationMode, setToolApprovalMode: setControllerToolApprovalMode, setComposerProfileForTab: setControllerComposerProfileForTab, setGoalForTab: setControllerGoalForTab, resumeGoalForTab: resumeControllerGoalForTab, pauseGoalForTab: pauseControllerGoalForTab, clearGoal: clearControllerGoal, clearGoalForTab: clearControllerGoalForTab, clearSession, listSessions, listTrashedSessions, resumeSession, openChannelSession, previewSession, deleteSession, restoreSession, purgeTrashedSession, renameSession, loadOlderHistory, retrySessionHistory, refreshMeta, pickWorkspace, switchWorkspace, rewindForTab, rewindForTabDetailed, undoRewindForTab, setModel, setEffort, setTokenMode, cancelJob, switchTab, openProjectTab, createDeliveryWorktree, openGlobalTab, closeTab, reorderTabs, openTopicSession, activateTopic, noteNavigationIntent, isNavigationIntentCurrent, syncActiveTab, ensureBlankTab, ensureBlankSurface, } = useController();
    ...
    const { mounted: terminalContentVisible } = useMountTransition(terminalPanelOpen, TERMINAL_CLOSE_TRANSITION_MS);
    ...
    useEffect(() => {
        ...
        const inspect = async () => {
            try {
                ...
                if (!disposed)
                    setWorkspaceConflict(conflict.state === "none" ? null : conflict);
            }
            catch {
                if (!disposed)
                    setWorkspaceConflict(null);
    ...
    useEffect(() => {
        if (pendingModeSwitch && pendingModeSwitch.tabId !== activeTabId)
    ...
    useEffect(() => {
        ...
        void app.Platform()
            .then((value) => {
            if (!cancelled)
                setDesktopPlatform(normalizeDesktopPlatform(value));
        })
            .catch((e) => {
    ...
    const applyDesktopPreferences = useCallback((settings: Pick<SettingsView, "desktopTheme" | "desktopThemeStyle" | "desktopTerminalTheme" | "desktopLayoutStyle" | "desktopLanguage" | "checkUpdates" | "statusBarStyle" | "statusBarItems" | "conversationWidth"> & {
    ...
    const commitThenSendRef = useRef<(tabId: string, displayText: string, submitText?: string, structured?: StructuredInvocationSubmit, initialGoal?: {
        goal: string;
    ...
    const handleInvocationMetadataChange = useCallback((metadata: InvocationMetadataMap) => {
        ...
        setInvocationMetadataByTab((current) => {
            ...
            if (names.length === Object.keys(previous).length && names.every((name) => (previous[name]?.kind === metadata[name]?.kind && previous[name]?.color === metadata[name]?.color)))
                return current;
    ...
    const workspacePanelRenderable = workspacePanelOpen && (workspacePanelMaximized ||
    ...
    const resolveLiveWorkspacePanelRenderWidth = useCallback((preferredWidth: number, nextSidebarWidth = sidebarWidth) => resolveLiveWorkspacePanelWidth({
        viewportWidth,
        sidebarCollapsed,
    ...
    const activeTab = useMemo(() => tabMetas.find((tab) => tab.id === activeTabId) ?? tabMetas.find((tab) => tab.active), [activeTabId, tabMetas]);
    const activePlanRevisionInsertRequest = planRevisionInsertRequest &&
    ...
    const sidebarImDetailConnection = useMemo(() => sidebarImConnections.find((connection) => connection.id === sidebarImDetailConnectionId) ?? null, [sidebarImConnections, sidebarImDetailConnectionId]);
    useEffect(() => {
        let cancelled = false;
        ...
        void app.GetTopicSummary({
            ...
            .then((topic) => {
            if (!cancelled)
                setActiveTopicTurns(topic.turns);
        })
            .catch(() => {
            if (!cancelled)
                setActiveTopicTurns(undefined);
    ...
    const backendActiveComposerProfile = useMemo(() => {
        if (state.meta) {
            return composerProfileFromMeta(state.meta, activeTab ? composerProfileMode(composerProfileFromTab(activeTab, activeComposerProfile?.toolApprovalMode)) : undefined, activeComposerProfile?.toolApprovalMode);
        }
    ...
    useEffect(() => {
        ...
        const frame = requestAnimationFrame(() => {
            if (decisionSurfaceRef.current != null)
                return;
            if (activeTabIdRef.current !== tabAtRelease)
                return;
    ...
    const submitExtensionForm = useCallback(async (values: Record<string, unknown>) => {
        ...
        if (!pending || !activeTabId || extensionFormBusy)
            return;
    ...
    const patchActiveComposerProfile = useCallback((patch: Partial<Omit<ComposerProfile, "pending">>, pendingFields: ComposerProfileField[]) => {
        if (!activeTabId)
            return;
    ...
    const patchComposerProfileForTab = useCallback((tabId: string, patch: Partial<Omit<ComposerProfile, "pending">>, pendingFields: ComposerProfileField[]) => {
        if (!tabId)
            return;
    ...
    useEffect(() => {
        if (!renamingTopicId || activeTab?.topicId === renamingTopicId)
            return;
    ...
    const applyCollaborationMode = useCallback(async (m: CollaborationMode): Promise<void> => {
        if (m === "goal") {
    ...
    const applyToolApprovalMode = useCallback((m: ToolApprovalMode) => {
        if (!activeTabId)
            return;
        if (m === "yolo") {
    ...
    const toggleYoloApprovalMode = useCallback(() => {
        if (!activeTabId)
            return;
        const next = toggleYoloToolApprovalMode(toolApprovalMode, yoloRestoreToolApprovalModesRef.current[activeTabId]);
        if (next.restore) {
    ...
    const patchActivatedGoalForTab = useCallback((tabId: string, nextGoal: string): void => {
    ...
    const applyGoalForTab = useCallback(async (tabId: string, nextGoal: string): Promise<void> => {
        if (!tabId)
            return;
        const trimmed = nextGoal.trim();
    ...
    const applyGoal = useCallback(async (nextGoal: string): Promise<void> => {
        if (!activeTabId)
            return;
        await applyGoalForTab(activeTabId, nextGoal);
    }, [activeTabId, applyGoalForTab]);
    const commitTokenModeSwitch = useCallback(async (tabId: string, m: TokenMode, previous: TokenMode, profile: ComposerProfile): Promise<void> => {
        if (!tabId || activeTabIdRef.current !== tabId || runtimeTransitionTabsRef.current.has(tabId))
            return;
    ...
    const applyTokenMode = useCallback(async (m: TokenMode): Promise<void> => {
        const tabId = activeTabId;
        if (!tabId || runtimeTransitionTabsRef.current.has(tabId) || m === composerProfile.tokenMode)
            return;
    ...
    const switchModel = useCallback(async (name: string) => {
        ...
        if (!activeTabId)
            return false;
        const profileApplied = await setControllerComposerProfileForTab(activeTabId, controllerComposerProfileCollaborationMode(composerProfile), toolApprovalMode, goal, { propagateError: true });
    ...
    useEffect(() => {
        if (!controllerReady || !activeTabId)
            return;
        runGoalAction(async () => {
            await setControllerComposerProfileForTab(activeTabId, controllerComposerProfileCollaborationMode(composerProfile), toolApprovalMode, goal, { propagateError: true });
        });
    ...
    const todos = useMemo(() => resolveTodoPanelTodos(metaTodos, todoItem ? parseTodos(todoItem.args) : undefined), [metaTodos, todoItem]);
    ...
    const todoScope = useMemo(() => todoPanelScope({ activeTab, activeTabId, eventChannel: state.meta?.eventChannel }), [activeTab, activeTabId, state.meta?.eventChannel]);
    const dismissedTodo = useMemo(() => dismissedTodoKeyForScope(todoScope, dismissedTodoKeys, todoKey), [dismissedTodoKeys, todoKey, todoScope]);
    ...
    const getSessionMarkdown = useCallback(() => sessionItemsToMarkdown(sessionTitle, state.items, liveStore.getSnapshot(activeTabId) ?? state.live), [activeTabId, liveStore, sessionTitle, state.items, state.live]);
    const getSessionJson = useCallback(() => sessionItemsToJson(sessionTitle, state.items, liveStore.getSnapshot(activeTabId) ?? state.live), [activeTabId, liveStore, sessionTitle, state.items, state.live]);
    useEffect(() => {
        if (!topicExportOpen)
            return;
        const onDown = (event: MouseEvent) => {
            const target = event.target as Element | null;
            if (!target?.closest(".topicbar__export"))
                setTopicExportOpen(false);
        };
        document.addEventListener("mousedown", onDown);
        return () => document.removeEventListener("mousedown", onDown);
    }, [topicExportOpen]);
    const exportSession = useCallback(async (format: "markdown" | "json" | "pdf" | "image") => {
        ...
        try {
            if (format === "json") {
                ...
                showToast(payloads.length > 1
                    ...
                    : t("topicBar.exportSuccess", { count: 1 }), "info");
            }
            else {
    ...
    useEffect(() => {
        ...
        if (!text || pendingPlanRevisionSendingTabsRef.current.has(activeTabId))
            return;
    ...
    const handleSend = useCallback(async (displayText: string, submitText = displayText, requestedTabId = activeTabId, structured?: StructuredInvocationSubmit) => {
        ...
        if (decisionMock === "workspace_conflict" || decisionMock === "mode_jobs" || decisionMock === "close_active" || decisionMock === "clear_context") {
            ...
            const mockWork: ActiveWorkView = {
                ...
                cancellable: true,
                jobs: [
                    { id: "mock-decision-build", kind: "bash", label: "pnpm build", status: "running", startedAt: Date.now() - 42000 },
                    { id: "mock-decision-test", kind: "bash", label: "go test ./...", status: "running", startedAt: Date.now() - 18000 },
                ],
            };
        ...
        if (collaborationMode === "goal" && !goal.trim()) {
            ...
            await activateGoalAndSubmitOnTab({
                ...
                submitText,
                structured,
                sendToTab: (tabId, nextGoal, display, routedSubmit, routedStructured) => commitThenSendRef.current(tabId, display, routedSubmit, routedStructured, {
                    goal: nextGoal,
        ...
        if (!controllerReady)
            return;
        const profileApplied = await setControllerComposerProfileForTab(sourceTabId, controllerComposerProfileCollaborationMode(composerProfile), toolApprovalMode, goal);
        if (!profileApplied)
            return;
    ...
    const refreshTabMetas = useCallback(async (apply?: () => boolean, options?: {
        afterMutation?: boolean;
    }): Promise<TabMeta[]> => {
        const result = await tabMetaRefreshCoordinatorRef.current!.run(async () => asArray(await app.ListTabs().catch(() => [] as TabMeta[])), options?.afterMutation ? { invalidate: true } : undefined);
        ...
        const unsub = onEvent((e) => {
            ...
            window.setTimeout(() => {
                ...
                refreshTabMetas(undefined, { afterMutation: true }).then((tabs) => {
                    ...
                    setComposerProfilesByTab((current) => patchComposerProfile(current, turnTabId, current[turnTabId] ?? baseProfile, { collaborationMode: "plan", goalDraftMode: false, goal: "" }, ["collaborationMode", "goal"]));
    ...
    useEffect(() => {
        const offStatus = onRemoteStatus((s) => {
            ...
            if (s.state === "stopped" && s.error)
                requestRemoteStatusPopover(s.hostId);
        });
    ...
    useEffect(() => {
        ...
        void app.RemoteConnectionStatuses()
            .then((statuses) => {
            if (!cancelled)
                hydrateRemoteStatuses(statuses);
        })
            .catch(() => { });
        return () => {
            cancelled = true;
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            frame = window.requestAnimationFrame(() => {
                ...
                if (Math.abs(footerHeightRef.current - next) < 2)
                    return;
    ...
    const resizeSidebarWithKeyboard = useCallback((event: KeyboardEvent<HTMLButtonElement>) => {
        if (sidebarCollapsed)
            return;
    ...
    const ensureWorkspacePanelWidth = useCallback((width: number) => {
        closeTransientOverlays();
        if (rightDockMode === "context")
            return;
        const next = clampRightDockPreviewWidth(width);
    ...
    const startWorkspacePanelResize = useCallback((event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    const resizeWorkspacePanelWithKeyboard = useCallback((event: KeyboardEvent<HTMLButtonElement>) => {
        if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
            event.preventDefault();
    ...
    const startTerminalResize = useCallback((event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    const resizeTerminalWithKeyboard = useCallback((event: KeyboardEvent<HTMLButtonElement>) => {
        if (!terminalPanelOpen)
            return;
    ...
    const openWorkspacePanel = useCallback((mode: RightDockMode = rightDockMode) => {
        closeTransientOverlays();
    ...
    const openTerminalForPath = useCallback((path = ".") => {
    ...
    const launchRemoteWorkspace = useCallback(async (host: RemoteHostView, requestSeq: number) => {
        ...
        if (!remoteWorkspaceLaunchGate.current.isCurrent(host.id, requestSeq))
            return;
    ...
    const handleWorkspacePreviewModeChange = useCallback((active: boolean) => {
        if (workspacePreviewActive === active)
            return;
        closeTransientOverlays();
    ...
    }) as CSSProperties, [chatReservedWidth, liveTerminalHeight, sidebarRenderWidth, terminalPanelOpen, terminalRenderHeight, workspacePanelRenderWidth]);
    ...
    const tabSwitchPendingRef = useRef<PendingNavigationRequest<{
        tabId: string;
        optimisticTab?: TabMeta;
        navigationIntentSeq: number;
    }> | null>(null);
    const enqueueTabSwitch = useCallback((tabId: string, optimisticTab?: TabMeta): Promise<void> => {
        ...
        return enqueueNavigationRequest({ seqRef: tabSwitchSeqRef, runningRef: tabSwitchRunningRef, pendingRef: tabSwitchPendingRef }, { tabId, optimisticTab, navigationIntentSeq }, async (request) => {
            if (!isNavigationIntentCurrent(request.navigationIntentSeq))
                return;
            await switchTab(request.tabId, request.optimisticTab, request.navigationIntentSeq);
            if (!isNavigationIntentCurrent(request.navigationIntentSeq))
                return;
            await refreshTabMetas(() => isNavigationIntentCurrent(request.navigationIntentSeq), { afterMutation: true });
        });
    ...
    const finishTabClose = useCallback(async (id: string, policy: "keep_running" | "stop_and_close"): Promise<boolean> => {
        closeTransientOverlays();
        ...
        setTabMetas((current) => {
            ...
            if (!closingTab.active && closingTab.id !== activeTabId)
                return remaining;
    ...
    const resolvePendingClose = useCallback(async (policy: "keep_running" | "stop_and_close") => {
        ...
        if (!request || request.stopping)
            return;
        if (policy === "stop_and_close")
            setPendingClose({ ...request, stopping: true });
        const closed = await finishTabClose(request.tabId, policy);
        if (closed)
            setPendingClose(null);
        else
            setPendingClose((current) => current?.tabId === request.tabId ? { ...current, stopping: false } : current);
    ...
    const hydratePlaceholderActive = Boolean(state.hydrating &&
    ...
    const commitThenSend = useCallback(async (sourceTabId: string, displayText: string, submitText?: string, structured?: StructuredInvocationSubmit, initialGoal?: {
        goal: string;
        collaborationMode: CollaborationMode;
        toolApprovalMode: ToolApprovalMode;
    }) => {
        ...
        if (sourceTab.readOnly)
            throw new Error(t("composer.readOnlyChannel"));
        if (sourceTab.ready !== true ||
    ...
    const handleTranscriptPrompt = useCallback((text: string) => {
        if (activeTabId && controllerReady)
            void commitThenSend(activeTabId, text).catch((err) => console.warn("Failed to submit transcript prompt", err));
    ...
    const handleMessageAction = useCallback((turn: number, scope: string) => {
        ...
        if (!sourceTabId || activeTab?.readOnly)
            return;
        if (hydratePlaceholderActive)
            return;
        if (scope === "fork") {
        ...
        for (let i = 0; i < items.length; i++) {
            if (items[i].kind === "user") {
                const item = items[i] as Extract<Item, {
                    kind: "user";
                }>;
        ...
        const userItem = items[boundaryIdx]?.kind === "user" ? items[boundaryIdx] as Extract<Item, {
            kind: "user";
        }> : undefined;
    ...
    const handleEditPrompt = useCallback(async (turn: number, displayText: string, submitText?: string): Promise<boolean> => {
        const sourceTabId = activeTabId;
        if (!sourceTabId || activeTab?.readOnly || !controllerReady || hydratePlaceholderActive || rewindStatesByTabRef.current[sourceTabId] || state.running || state.messageAction != null || state.approval != null || state.ask != null || clearContextPending)
            return false;
    ...
    const refreshHistoryView = useCallback(async () => {
        ...
        if (!sessions)
            return;
        setHistView((cur) => cur === null || cur.kind !== "history"
            ? cur
            : cur.source === "scope"
    ...
    const runNavigationRequest = useCallback(async (request: PendingDesktopNavigationRequest) => {
        ...
        const openTopicTarget = async (scope: string, workspaceRoot: string, topicId: string, sessionPath?: string): Promise<TabMeta> => {
            if (singleSurfaceLayout)
                return activateTopic(scope, workspaceRoot, topicId, sessionPath || "", request.navigationIntentSeq);
            if (sessionPath)
                return openTopicSession(scope, workspaceRoot, topicId, sessionPath, request.navigationIntentSeq);
            if (scope === "global")
                return openGlobalTab(topicId, request.navigationIntentSeq);
        ...
        try {
            ...
            if (request.kind === "delivery-worktree") {
                ...
                if (!latest())
                    return;
                showToast(result.sourceDirty
                    ? t("projectTree.worktreeCreatedDirty", { branch: result.branch })
                    : t("projectTree.worktreeCreated", { branch: result.branch }), result.sourceDirty ? "warn" : "info", { durationMs: result.sourceDirty ? 7000 : 3500 });
                setTabRevealSignal((signal) => signal + 1);
                setTranscriptRevealSignal((signal) => signal + 1);
                return;
            }
            ...
            if (!latest() || isMissingSessionError(err))
                return;
            setHistView(null);
    ...
    const enqueueNavigationWithIntent = useCallback((input: DesktopNavigationIntent, navigationIntentSeq: number): Promise<void> => {
        return enqueueNavigationRequest({ seqRef: navigationSeqRef, runningRef: navigationRunningRef, pendingRef: navigationPendingRef }, { ...input, navigationIntentSeq } as DesktopNavigationInput, runNavigationRequest);
    ...
    const openBlankSession = useCallback((scope: string, workspaceRoot: string): Promise<void> => enqueueNavigation({ kind: "blank", scope, workspaceRoot: scope === "project" ? workspaceRoot : "" }), [enqueueNavigation]);
    ...
    const paletteItems = useMemo<PaletteItem[]>(() => {
        ...
        const remoteItems: PaletteItem[] = remoteHosts.map((host) => {
            ...
            return {
                ...
                run: () => {
                    if (connected)
                        openRemoteWorkspaceFromStatus(host);
                    else
                        connectAndOpenRemoteWorkspace(host);
                },
            };
        });
    ...
    const onDeleteManySessions = useCallback(async (paths: string[]) => {
        if (state.running)
            return;
    ...
    const onPurgeTrashedSession = useCallback(async (path: string) => {
        await purgeTrashedSession(path);
        const trashed = await listTrashedSessions();
        setHistView((cur) => (cur === null ? null : { kind: "trash", sessions: trashed }));
    }, [purgeTrashedSession, listTrashedSessions]);
    const onPurgeAllTrashedSessions = useCallback(async (paths: string[]) => {
        const uniquePaths = Array.from(new Set(paths));
        for (const path of uniquePaths) {
            await purgeTrashedSession(path);
        }
    ...
    const onPurgeRecoveryCopies = useCallback(async (paths: string[]) => {
        const uniquePaths = Array.from(new Set(paths));
        for (const path of uniquePaths) {
    ...
    const commitActiveTopicRename = useCallback(async () => {
        ...
        if (topicRenameCommitHandledRef.current)
            return;
        topicRenameCommitHandledRef.current = true;
    ...
    const handleChromeTitlebarDoubleClick = useCallback((event: ReactMouseEvent<HTMLDivElement>) => {
        ...
        if (!onChromeSurface && !onMacOSWorkbenchSidebarTitlebar)
            return;
        if (target?.closest("button, input, textarea, select, a, [role='button'], [role='tab'], .windows-window-controls"))
            return;
        event.preventDefault();
    ...
    <ShellHotkeys />
    <TextSizeHotkeys />
      <div ref={appRef} onDoubleClickCapture={handleChromeTitlebarDoubleClick} className={[
            "app",
            `app--${desktopPlatform}`,
      ...
      <div ref={layoutRef} className={[
            ...
            .filter(Boolean)
            .join(" ")} style={layoutStyle}>
        {!appChromeHidden && (<AppChrome platform={desktopPlatform} browserPreviewChrome={browserPreviewChrome} workbenchChrome={sidebarWorkbench} tabs={visibleTabs} activeTabId={visibleTabId} revealActiveSignal={tabRevealSignal} commandCompact={true} sidebarTogglePressed={sidebarTogglePressed} sidebarExpandBlocked={sidebarExpandBlocked} sidebarCollapsed={sidebarCollapsed} sidebarToggleTitle={sidebarToggleTitle} workspacePanelMaximized={workspacePanelMaximized} workspacePanelRenderable={workspacePanelRenderable} workspaceTogglePressed={workspaceTogglePressed} workspacePanelLabel={workspacePanelRenderable ? t("rightDock.collapse") : t("rightDock.expand")} onToggleSidebar={toggleSidebar} onToggleWorkspacePanel={toggleWorkspacePanel} onTabChange={(id) => void handleTabChange(id)} onTabClose={(id) => void handleTabClose(id)} onTabsClose={(ids, nextActiveTabId) => void handleTabsClose(ids, nextActiveTabId)} onTabsReorder={(ids) => void handleTabsReorder(ids)} onNewTab={() => void handleNewTab()} onOpenPalette={() => void openPalette()}/>)}
        ...
        <aside className={sidebarClassName} aria-label={t("sidebar.navigation")}>
          {sidebarWorkbench ? (<>
              ...
              <div className="sidebar__quick-actions">
                <button className="sidebar__quick-action" type="button" onClick={() => {
                void handleNewTab();
            }}>
          ...
          <section className="sidebar__section sidebar__section--projects">
            <ProjectTree activeScope={activeTab?.scope} activeWorkspaceRoot={activeTab?.workspaceRoot} activeTopicId={activeTab?.topicId} activeSessionPath={activeTab?.sessionPath} imTopicSources={imTopicSources} onOpenTopic={handleOpenTopic} onCreateTopic={(scope, workspaceRoot) => openBlankSession(scope, scope === "project" ? workspaceRoot : "")} onCreateDeliveryWorktree={(workspaceRoot) => enqueueNavigation({ kind: "delivery-worktree", workspaceRoot })} onTopicsChanged={refreshProjectsAndTabs} onRenameTopic={renameTopic} refreshSignal={projectRevision} onAddProject={async () => {
            await switchFolder();
          ...
          {sidebarWorkbench ? (<nav className="sidebar__nav sidebar__nav--footer">
              <div className="sidebar__utility-row" aria-label={t("sidebar.utilityActions")}>
                ...
                <Tooltip label={t("topbar.settings")} fill side="top">
                  <button className="sidebar__utility-button" type="button" onClick={() => {
                closeTransientOverlays();
                setSettingsTarget("general");
            }}>
        ...
        <section className={`chat-pane${creationEmptyHero ? " chat-pane--creation-empty" : ""}`}>
          ...
          <header className="topicbar">
            ...
            ].filter(Boolean).join(" ")} type="button" onClick={sidebarExpandBlocked ? undefined : toggleSidebar} aria-label={sidebarToggleTitle} aria-pressed={!sidebarCollapsed} aria-disabled={sidebarExpandBlocked}>
                  <PanelLeft size={15}/>
                </button>
              </Tooltip>)}
            <div className="topicbar__identity">
              <div className="topicbar__title-row">
                ...
                if (event.key === "Escape") {
                  ...
                  </h1>) : (<h1 title={sidebarImDetailConnection ? topicbarTitle : topicTitle(activeTab)}>{topicbarTitle}</h1>)}
              ...
              {topicbarSubtitleVisible && (<div className="topicbar__subtitle" title={topicbarSubtitleTitle}>
                  ...
                  {topicbarImSourcePlatform && (<span className={`topicbar__source-chip topicbar__source-chip--${topicbarImSourcePlatform}`}>
            ...
            <div className="topicbar__actions">
              {sidebarCreation && shouldMountExternalOpener(activeTab, Boolean(sidebarImDetailConnection)) && activeTab && (<ExternalOpener key={activeTab.id} tabId={activeTab.id} dismissSignal={transientOverlayDismissSignal}/>)}
              {!sidebarImDetailConnection && (<>
              ...
              <div className={`topicbar__export${topicExportOpen ? " topicbar__export--open" : ""}`}>
                <Tooltip label={t("topicBar.export")}>
                  <button className="topicbar__action-btn topicbar__action-btn--icon topicbar__action-btn--utility" type="button" disabled={!sessionHasContent} aria-label={t("topicBar.export")} aria-haspopup="menu" aria-expanded={topicExportOpen} onClick={() => setTopicExportOpen((open) => !open)}>
                    <Download size={14}/>
                  </button>
                </Tooltip>
                {topicExportOpen && (<div className="topicbar__export-menu" role="menu">
              ...
              {!sidebarImDetailConnection && (<Tooltip label={t("rightDock.terminal")}>
            ...
            ].filter(Boolean).join(" ")} type="button" onClick={toggleWorkspacePanel} aria-label={workspacePanelRenderable ? t("rightDock.collapse") : t("rightDock.expand")} aria-pressed={workspacePanelRenderable}>
                    <PanelRight size={15}/>
                  </button>
                </Tooltip>)}
              {tasksOpen && (<div className="taskmonitor-popover" role="dialog" aria-label={t("summary.session")}>
                  <Suspense fallback={null}>
                    <TaskMonitorPanel key={`${activeTab?.id || activeTabId || "none"}:${activeTab?.workspaceRoot || "global"}:${activeTab?.sessionPath || ""}:${tasksOpen}`} tabID={activeTab?.id || activeTabId || ""} initialOpen initialScope={tasksOpen || "session"} popover summaryMode onClose={() => setTasksOpen(false)} onOpenSession={openTaskMonitorSession}/>
                  </Suspense>
                </div>)}
            </div>
          </header>

          {state.meta?.startupErr && (<div className="banner banner--error">{t("topbar.startupError", { msg: state.meta.startupErr })}</div>)}
          {configLoadWarnings.length > 0 && (<div className="banner banner--warning banner--actionable">
              <span className="banner__msg" title={configLoadWarnings.join("\n")}>
                {t("config.loadWarning", { msg: configLoadWarnings[0] })}
              </span>
              <span className="banner__spacer"/>
              <button type="button" className="btn btn--small" onClick={() => void app.OpenUserConfigPath?.().catch(() => { })}>
                {t("config.openConfig")}
              </button>
          ...
          {providerSetupNeeded && !needsOnboarding && (<div className="banner banner--warning banner--actionable">
          ...
          <UpdateBanner enabled={startupUpdateChecksEnabled === true} onShowReleaseNotes={(latest) => {
            const version = latest.replace(/^(?:desktop-)?v/, "");
            void openExternal(`https://reasonix.io/changelog/v${version}/`);
        }}/>

          <main className="main">
            {sidebarImDetailConnection ? (<SidebarImConnectionDetail connection={sidebarImDetailConnection} onClose={() => setSidebarImDetailConnectionId("")} onOpenSettings={openBotSettings} onManageAllowlist={() => openBotAllowlistSettings(sidebarImDetailConnection.connectionId)} onOpenSession={() => void openSidebarImConnectionSession(sidebarImDetailConnection)}/>) : noticePreviewMockEnabled() ? (<NoticePreviewPanel />) : (<>
          ...
          {!sidebarImDetailConnection && (<footer className={["footer", terminalPanelOpen && !sidebarCreation ? "footer--compact" : "", decisionSurface ? "footer--decision" : ""].filter(Boolean).join(" ")} ref={footerRef}>
            ...
            {decisionSurface === "tool_approval" || decisionSurface === "plan_approval"
                ? state.approval && (<ApprovalModal key={`${activeTabId ?? ""}:${state.approval.id}`} approval={state.approval} cwd={state.meta?.cwd} tabId={activeTabId} workspaceScopeKey={workspaceScopeKey} insertRequest={activePlanRevisionInsertRequest} onRevisionActiveChange={handleRevisionActiveChange} onAnswer={async (allow, session, persist) => {
                ...
                : decisionSurface === "ask"
                    ? state.ask && (<AskCard key={`${activeTabId ?? ""}:${state.ask.id}`} ask={state.ask} onAnswer={answerQuestion} onDismiss={() => answerQuestion(state.ask!.id, [])} onStop={() => {
                            cancel();
                        }}/>)
                    : decisionSurface === "extension_form"
                        ? state.extensionForm && (<ExtensionFormDialog key={`${activeTabId ?? ""}:${state.extensionForm.pluginId}:${state.extensionForm.surfaceId}`} surface={state.extensionForm} busy={extensionFormBusy} onSubmit={(values) => void submitExtensionForm(values)} onCancel={() => void cancelExtensionForm()}/>)
                        : decisionSurface === "workspace_conflict" && workspaceConflict ? (<RuntimeDecisionCard id="workspace-conflict" title={t("runtime.workspaceConflictTitle")} badge={t("runtime.workspaceConflictBadge")} meta={workspaceConflict.state === "local"
                                ? t("runtime.workspaceConflictLocal", { title: workspaceConflict.ownerTitle || t("runtime.unknownTask") })
                                : t("runtime.workspaceConflictExternal")} note={t("runtime.workspaceConflictNote")} onCancel={() => {
                                cancel();
                                setWorkspaceConflict(null);
                            }} actions={[
                            ...
                            : decisionSurface === "mode_jobs" && pendingModeSwitch ? (<RuntimeDecisionCard id="mode-jobs" title={t("runtime.modeJobsTitle", { mode: t(runtimeProfileShortKey(pendingModeSwitch.target)) })} badge={t("status.jobs", { n: pendingModeSwitch.work.jobs.length })} meta={t("runtime.modeJobsMeta")} note={pendingModeSwitch.work.jobs.map((job) => job.label || job.kind).join(" · ")} onCancel={() => setPendingModeSwitch(null)} actions={[
                                    {
                                ...
                                : decisionSurface === "close_active" && pendingClose ? (<RuntimeDecisionCard id="close-active" title={t("runtime.closeTitle")} badge={t("status.jobs", { n: pendingClose.work.jobs.length })} meta={t("runtime.closeMeta")} onCancel={() => setPendingClose(null)} actions={[
                                        {
                                    ...
                                    : decisionSurface === "clear_context" ? (<ClearContextCard onCancel={cancelClearContext} onConfirm={() => {
        ...
        {workspacePanelGridOpen && (<button className="workspace-panel-resizer" type="button" role="separator" aria-orientation="vertical" aria-label={t("rightDock.resize")} aria-valuemin={workspacePanelResizeMinWidth} aria-valuemax={Math.max(workspacePanelMaxWidth, workspacePanelRenderWidth)} aria-valuenow={workspacePanelRenderWidth} onPointerDown={startWorkspacePanelResize} onKeyDown={resizeWorkspacePanelWithKeyboard} onDoubleClick={() => setSavedWorkspacePanelWidth(workspacePanelResetWidth)}/>)}

        {workspacePanelRenderable && (<aside className={[
                "workbench-dock",
            ...
            <div className="workbench-dock__tools">
              <div className="workbench-dock__tabs" role="tablist" aria-label={t("rightDock.views")}>
                ...
                <button type="button" role="tab" aria-selected={rightDockMode === "files"} className={`workbench-dock__tab${rightDockMode === "files" ? " workbench-dock__tab--active" : ""}`} onClick={() => openRightDockMode("files")}>
                  <FileText size={13}/>
                  <span className="workbench-dock__tab-label">{t("workspace.filesTab")}</span>
                </button>
                <button type="button" role="tab" aria-selected={rightDockMode === "changed"} className={`workbench-dock__tab${rightDockMode === "changed" ? " workbench-dock__tab--active" : ""}`} onClick={() => openRightDockMode("changed")}>
                  <GitBranch size={13}/>
                  <span className="workbench-dock__tab-label">{t("workspace.changedTab")}</span>
                </button>
                {remoteHosts.length > 0 && (<button type="button" role="tab" aria-selected={rightDockMode === "remote"} className={`workbench-dock__tab${rightDockMode === "remote" ? " workbench-dock__tab--active" : ""}`} onClick={openRemoteDock}>
                    <Server size={13}/>
            ...
            <div className="workbench-dock__body">
              {rightDockMode === "remote" ? (<Suspense fallback={null}>
                  <RemotePanel onClose={() => setWorkspacePanel(false)}/>
                </Suspense>) : rightDockMode === "context" && desktopLayoutStyle !== "creation" ? (<Suspense fallback={null}>
        ...
        <>
          <aside className="terminal-drawer" aria-label={t("terminal.title")}>
            {terminalContentVisible && (<Suspense fallback={<div className="terminal-empty"><span className="terminal-empty__spinner"/>{t("terminal.loading")}</div>}>
                <TerminalPanel tabId={activeTabId ?? ""} cwd={state.meta?.cwd} readOnly={Boolean(activeTab?.readOnly)} onClose={() => {
                setTerminalPanelOpen(false);
                saveTerminalPanelOpen(false);
            }} onAddOutput={(sessionId) => void addTerminalOutputToComposer(sessionId)}/>
              </Suspense>)}
          </aside>
          <button className="terminal-drawer-resizer" type="button" role="separator" aria-orientation="horizontal" aria-label={t("terminal.resize")} aria-valuemin={TERMINAL_MIN_HEIGHT} aria-valuemax={terminalResizeMaxHeight} aria-valuenow={liveTerminalHeight ?? terminalRenderHeight} aria-hidden={!terminalPanelOpen} tabIndex={terminalPanelOpen ? 0 : -1} onPointerDown={startTerminalResize} onKeyDown={resizeTerminalWithKeyboard} onDoubleClick={() => {
            setSavedTerminalHeight(TERMINAL_DEFAULT_HEIGHT);
        }}/>
        </>

        {!sidebarImDetailConnection && (<StatusBar context={state.context} usage={state.usage} balance={state.balance} running={state.running || rewindCommitting} jobs={state.jobs} onCancelJob={cancelJob} backgroundRuntimes={backgroundRuntimes} onCancelRuntimeJob={cancelRuntimeJob} onRevealRuntime={revealBackgroundRuntime} sessionTurns={sessionTurns} sessionTokens={state.sessionTokens} turnTokens={state.turnTotalTokens} lastTurnOutputTokens={state.lastTurnOutputTokens} lastTurnModelMs={state.lastTurnModelMs} lastTurnOutputEstimated={state.lastTurnOutputEstimated} lastRequestTps={state.lastRequestTps} turnCost={state.turnCost} cost={state.sessionCost} currency={state.sessionCurrency} modelLabel={state.meta?.label} labelStyle={statusBarStyle} items={statusBarItems} extensionStatuses={extensionStatusList} workspacePath={state.meta?.workspacePath || state.meta?.workspaceRoot || state.meta?.cwd} workspaceName={state.meta?.workspaceName} gitBranch={state.meta?.gitBranch} onConnectRemote={connectAndOpenRemoteWorkspace} onDisconnectRemote={(hostId) => void app.DisconnectRemoteHost(hostId).catch(() => { })} onManageRemote={() => setSettingsTarget("remote")} onOpenRemote={requestRemoteExplorer} onOpenRemoteWorkspace={openRemoteWorkspaceFromStatus} remoteHosts={remoteHosts} remoteStatuses={remoteStatuses}/>)}
      </div>

      {histView !== null && (<Suspense fallback={null}>
          <HistoryPanel kind={histView.kind} sessions={histView.sessions} running={state.running} onResume={onResumeSession} onPreview={previewSession} onDelete={onDeleteSession} onRename={onRenameSession} onRestore={onRestoreTrashedSession} onPurge={onPurgeTrashedSession} onPurgeAll={onPurgeAllTrashedSessions} onPurgeRecoveryCopies={onPurgeRecoveryCopies} onDeleteMany={onDeleteManySessions} onClose={closeHistory}/>
        </Suspense>)}

      {settingsTarget !== null && (<Suspense fallback={null}>
          <SettingsPanel initialTab={settingsTarget} initialFocus={settingsFocus ?? undefined} agentRunning={state.running} desktopPlatform={desktopPlatform} activeWorkspaceKey={`${activeTab?.id ?? activeTabId ?? ""}\u0000${activeTab?.workspaceRoot ?? activeTab?.cwd ?? state.meta?.cwd ?? ""}`} onUseSubagent={prefillSubagentCommand} onClose={() => {
                setSettingsFocus(null);
      ...
      <CommandPalette open={paletteOpen} onClose={() => setPaletteOpen(false)} items={paletteItems} placeholder={t("palette.placeholder")} emptyText={t("palette.empty")}/>

      <ShortcutsCheatsheet open={shortcutsOpen} platform={desktopPlatform} onClose={() => setShortcutsOpen(false)} t={t}/>

      {startupSplashVisible && (<StartupSplash hold={startupSplashHold} onDone={() => setStartupSplashVisible(false)}/>)}

      {needsOnboarding && (<OnboardingOverlay onComplete={() => {
      ...
      <Suspense fallback={null}><TranscriptSelectionMenu enabled={Boolean(activeTabId && !activeTab?.readOnly && !decisionSurface && !sidebarImDetailConnection && !hydratePlaceholderActive)} resetKey={activeTabId ?? ""} onAddToChat={addSelectedTextToComposer}/>
      </Suspense>
...
export type { SidebarImConnection as SidebarImConnection } from "./app_sidebar";
export type { SidebarImTopicSource as SidebarImTopicSource } from "./app_sidebar";
export { sidebarImConnectionsFromBot as sidebarImConnectionsFromBot } from "./app_sidebar";
export { taskSessionIDFromPath as taskSessionIDFromPath } from "./app_sidebar";
export { sidebarImSessionTarget as sidebarImSessionTarget } from "./app_sidebar";
export { isChannelSession as isChannelSession } from "./app_sidebar";
export { sidebarImTopicSourcesFromBot as sidebarImTopicSourcesFromBot } from "./app_sidebar";
export { sidebarImScopeLabel as sidebarImScopeLabel } from "./app_sidebar";
export { SidebarImConnectionDetail as SidebarImConnectionDetail } from "./app_sidebar";
export { CHAT_MIN_WIDTH as CHAT_MIN_WIDTH } from "./app_window";
export { CHAT_COMFORT_MIN_WIDTH as CHAT_COMFORT_MIN_WIDTH } from "./app_window";
export { WORKSPACE_RESIZER_WIDTH as WORKSPACE_RESIZER_WIDTH } from "./app_window";
export type { DesktopPlatform as DesktopPlatform } from "./app_window";
export { isMacOSWorkbenchSidebarTitlebar as isMacOSWorkbenchSidebarTitlebar } from "./app_window";
export { useWindowsMaximised as useWindowsMaximised } from "./app_window";
export { WindowsWindowControls as WindowsWindowControls } from "./app_window";
export { normalizeDesktopPlatform as normalizeDesktopPlatform } from "./app_window";
export { browserPlatformOverride as browserPlatformOverride } from "./app_window";
export type { DecisionSurfaceKind as DecisionSurfaceKind } from "./app_helpers";
export { TERMINAL_CLOSE_TRANSITION_MS as TERMINAL_CLOSE_TRANSITION_MS } from "./app_helpers";
export { noticePreviewMockEnabled as noticePreviewMockEnabled } from "./app_helpers";
export { runtimeProfileShortKey as runtimeProfileShortKey } from "./app_helpers";
export { NoticePreviewPanel as NoticePreviewPanel } from "./app_helpers";
...
export type { DesktopNavigationIntent as DesktopNavigationIntent } from "./app_helpers";
export type { DesktopNavigationInput as DesktopNavigationInput } from "./app_helpers";
export type { PendingDesktopNavigationRequest as PendingDesktopNavigationRequest } from "./app_helpers";
export { loadDismissedTodoKeys as loadDismissedTodoKeys } from "./app_helpers";
export { saveDismissedTodoKeys as saveDismissedTodoKeys } from "./app_helpers";
export { GUIDANCE_QUEUE_MOCK_ITEMS as GUIDANCE_QUEUE_MOCK_ITEMS } from "./app_helpers";
export { browserMockScenarioParam as browserMockScenarioParam } from "./app_helpers";
export { isGuidanceMockScenario as isGuidanceMockScenario } from "./app_helpers";
export { detectBrowserPlatform as detectBrowserPlatform } from "./app_helpers";
export { tabWorkspaceTitle as tabWorkspaceTitle } from "./app_helpers";
export { topicTitle as topicTitle } from "./app_helpers";
export { topicDisplayTitle as topicDisplayTitle } from "./app_helpers";
export { sessionsForScope as sessionsForScope } from "./app_helpers";
export { isMissingSessionError as isMissingSessionError } from "./app_helpers";
export { workspaceDisplayName as workspaceDisplayName } from "./app_helpers";
export { sessionItemsToMarkdown as sessionItemsToMarkdown } from "./app_helpers";
export { sessionItemsToJson as sessionItemsToJson } from "./app_helpers";
export { safeFilename as safeFilename } from "./app_helpers";
export { ShellHotkeys as ShellHotkeys } from "./app_helpers";
export { TextSizeHotkeys as TextSizeHotkeys } from "./app_helpers";
