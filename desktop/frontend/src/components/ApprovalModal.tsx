...
import { DecisionConfirmBar, PromptAction, PromptBadge, PromptDescriptionDisclosure, PromptHeaderAction, PromptShelf } from "./PromptShelf";
import { animateElementExit, DUR_FAST } from "../lib/motion";
import { FileReferenceMenu, insertTextAtSelection, pickInlineFileReference, useFileReferenceMenu } from "./FileReferenceMenu";
import { approvalToolLabel, requiresFreshHumanApproval, APPROVAL_MODE_RANK, localizeApprovalSubject, localizeApprovalReason, localizePlanModeApprovalReason, DecisionAction, RECOVERY_FEEDBACK_MAX, recoveryReasonText, planDelta } from "./approval_modal_helpers";
export function ApprovalModal({ approval, onAnswer, onResolveRecovery, onRevisePlan, onExitPlan, onStop, cwd, tabId, workspaceScopeKey, insertRequest, onRevisionActiveChange, toolApprovalMode, }: {
    ...
    const isRecoveryPlanChange = isRecoveryApproval && (recoveryChangeKind === "strategy" || recoveryChangeKind === "scope");
    ...
    const approvalModeRelaxed = !isPlanApproval &&
    ...
    const [reasonOpen, setReasonOpen] = useState(() => {
        if (isRecoveryApproval)
            return false; // recovery details stay collapsed
    ...
    const answerWithExit = (fn: () => void) => {
        if (closingRef.current || submitting)
            return;
    ...
    const resolveRecovery = useCallback((action: "continue" | "continue_task" | "revise", feedback?: string) => {
    ...
    const confirmSelected = useCallback(() => {
        if (submitting || closingRef.current)
            return;
        if (isPlanApproval || isRecoveryApproval)
            return;
        ...
        const onKeyDown = (event: globalThis.KeyboardEvent) => {
            ...
            if (isRecoveryApproval && recoveryGuidanceOpen && event.key === "Escape") {
                ...
                requestAnimationFrame(() => {
                    if (isRecoveryPlanChange)
                        cardRef.current?.focus();
                    else
                        recoveryGuidanceTriggerRef.current?.focus();
                });
                return;
            }
            ...
            if (tag === "input" || tag === "textarea" || tag === "select" || (target instanceof HTMLElement && target.isContentEditable))
                return;
            ...
            if (event.key === "ArrowUp") {
                ...
                if (isRecoveryApproval && selectedIndexRef.current < 0)
                    return;
                event.preventDefault();
                confirmSelected();
            }
            else if (event.key === "1" || event.key === "2" || event.key === "3" || event.key === "4") {
                if (isRecoveryApproval && recoveryGuidanceOpen)
                    return;
    ...
    useEffect(() => () => {
        if (revisionActiveRef.current)
            onRevisionActiveChangeRef.current?.(false);
    }, []);
    ...
    useEffect(() => {
        if (!insertRequest || insertRequest.id === consumedInsertIdRef.current)
            return;
    ...
    const recoveryReason = isRecoveryApproval
        ? recoveryReasonText(recovery?.change_kind, recovery?.change_rationale || recovery?.review_rationale || reason, t)
        : "";
    const recoveryActionSummary = recovery?.next_action ||
        recovery?.next_tool ||
        subjectSummary ||
        approval.tool;
    const recoveryPlanDelta = isRecoveryPlanChange
        ...
        : null;
    const hasRecoveryDetails = Boolean(recovery?.failed_summary ||
        recovery?.diagnosis ||
        recovery?.change_rationale ||
        recovery?.review_rationale ||
        recovery?.source_agent);
    const confirmIsDanger = selectedAction?.tone === "danger";
    const confirmLabel = selectedAction?.kind === "toggle-revision"
        ? revisionOpen
            ? t("common.cancel")
            : t("approval.revisePlan")
        : t("decision.confirm");
    return (<div ref={shelfRef}>
      <PromptShelf decision actionsRole={isPlanApproval || isRecoveryApproval ? "group" : "listbox"} className={isPlanApproval ? "prompt-shelf--plan-approval" : isRecoveryApproval ? "prompt-shelf--recovery-approval" : "prompt-shelf--tool-approval"} barRef={cardRef} titleId={isPlanApproval ? "plan-approval-title" : isRecoveryApproval ? "recovery-approval-title" : "tool-approval-title"} title={isPlanApproval
            ? t("approval.planReady")
            : isRecoveryPlanChange
                ? t("approval.recoveryPlanChangePending")
                : isRecoveryApproval
                    ? t("approval.recoveryPending")
                    : t("approval.toolPending")} badges={<>
            ...
            {isRecoveryPlanChange && (<PromptBadge>
                {t(recoveryChangeKind === "strategy" ? "approval.recoveryDecisionStrategy" : "approval.recoveryDecisionScope")}
              </PromptBadge>)}
          </>} meta={isRecoveryApproval ? undefined : toolMeta} headerActions={<>
            {isRecoveryApproval && hasRecoveryDetails && (<PromptHeaderAction onClick={() => setReasonOpen((open) => !open)} disabled={submitting}>
                {t(reasonOpen ? "approval.recoveryHideTechnicalDetails" : "approval.recoveryTechnicalDetails")}
              </PromptHeaderAction>)}
            {!isPlanApproval && !isRecoveryApproval && hasToolDetails && reason && (<PromptHeaderAction onClick={() => setReasonOpen((open) => !open)} disabled={submitting}>
                {t(reasonOpen ? "approval.hideDetails" : "approval.details")}
              </PromptHeaderAction>)}
            {!isPlanApproval && !isRecoveryApproval && (<PromptHeaderAction onClick={() => answerWithExit(onStop)} ariaLabel={t("decision.stopTask")} disabled={submitting}>
                {t("decision.stopTask")}
              </PromptHeaderAction>)}
          </>} actions={<>
            {toolActions.map((action, index) => {
                const actionNode = (<PromptAction key={action.key} keyLabel={action.key} label={action.label} description={action.desc} descriptionId={`${instanceId}-description-${index}`} descriptionDisclosure onDescriptionOverflowChange={!isPlanApproval && !isRecoveryApproval && selectedIndex === index
                        ? setDescriptionTruncated
                        : undefined} onClick={() => {
                        activateAction(action, index);
                    }} primary={action.primary} selected={selectedIndex === index} tone={action.tone} role={isPlanApproval || isRecoveryApproval ? "button" : "option"} disabled={submitting}/>);
                if (isRecoveryApproval && !isRecoveryPlanChange && index === 1 && recovery?.can_grant_task) {
                    return (<div key={action.key} className={[
                            "recovery-continue-option",
                            grantSimilarForTask ? "recovery-continue-option--granted" : "",
                        ].filter(Boolean).join(" ")}>
                    {actionNode}
                    {!recoveryGuidanceOpen && (<label className="recovery-task-grant">
                        <input type="checkbox" checked={grantSimilarForTask} onChange={(event) => setGrantSimilarForTask(event.target.checked)} disabled={submitting}/>
                        <span>
          ...
          </>} note={!isPlanApproval && !isRecoveryApproval && selectedDescriptionId && descriptionTruncated ? (<PromptDescriptionDisclosure descriptionId={`${selectedDescriptionId}-detail`} label={selectedAction?.label} description={selectedAction?.desc ?? ""} expanded={descriptionExpanded} onToggle={() => setExpandedDescriptionId((current) => current === selectedDescriptionId ? null : selectedDescriptionId)} disabled={submitting}/>) : isRecoveryApproval ? (recoveryGuidanceOpen ? (<div className="recovery-guidance">
                <textarea ref={recoveryGuidanceRef} className="plan-revision__input recovery-guidance__input" value={recoveryGuidanceText} rows={3} maxLength={RECOVERY_FEEDBACK_MAX} aria-label={t("approval.recoveryGuidanceLabel")} placeholder={t("approval.recoveryGuidancePlaceholder")} onChange={(event) => setRecoveryGuidanceText(event.target.value.slice(0, RECOVERY_FEEDBACK_MAX))} onKeyDown={onRecoveryGuidanceKeyDown} disabled={submitting} autoFocus/>
                <div className="recovery-guidance__actions">
                  ...
                  <button className="btn btn--primary" type="button" onClick={submitRecoveryGuidance} disabled={submitting || !recoveryGuidanceText.trim()}>
                    {t(isRecoveryPlanChange ? "approval.recoveryPlanGuidanceSubmit" : "approval.recoveryGuidanceSubmit")}
                  </button>
                </div>
              </div>) : isRecoveryPlanChange ? undefined : (<button ref={recoveryGuidanceTriggerRef} type="button" className="recovery-guidance-trigger" aria-expanded="false" onClick={() => {
              ...
              </button>)) : undefined} footer={isRecoveryApproval || isPlanApproval ? undefined : (<DecisionConfirmBar hint={t("decision.selectHint")} confirmLabel={confirmLabel} onConfirm={confirmSelected} disabled={submitting} danger={confirmIsDanger}/>)}>
        {(approvalModeRelaxed ||
            isRecoveryApproval ||
            (!isPlanApproval && !isRecoveryApproval && (subject || (reasonOpen && reason))) ||
            (isPlanApproval && revisionOpen)) && (<>
            {approvalModeRelaxed && !isRecoveryApproval && (<div className="approval-mode-hint">{t("approval.modeSwitchPendingHint")}</div>)}
            {isRecoveryApproval && (<section className="recovery-summary" aria-label={t("approval.recoverySummaryLabel")}>
                <p className="recovery-summary__reason">{recoveryReason}</p>
                {!isRecoveryPlanChange && recoveryActionSummary && (<p className="recovery-summary__action">
                    <span>{t("approval.recoveryNextLabel")}</span>
                    <code>{recoveryActionSummary}</code>
                  </p>)}
              </section>)}
            {isRecoveryPlanChange && recoveryPlanDelta && (<section className="plan-change-delta" aria-label={t("approval.recoveryPlanDeltaLabel")}>
                <div className="plan-change-delta__title">{t("approval.recoveryPlanDeltaLabel")}</div>
                {recoveryPlanDelta.removed.length > 0 && (<div className="plan-change-delta__group plan-change-delta__group--removed">
                    <div className="plan-change-delta__label">{t("approval.recoveryPlanRemoved")}</div>
                    {recoveryPlanDelta.removed.map((line, index) => (<div className="plan-change-delta__line" key={`removed-${index}-${line}`}><span>−</span>{line}</div>))}
                  </div>)}
                {recoveryPlanDelta.added.length > 0 && (<div className="plan-change-delta__group plan-change-delta__group--added">
                    <div className="plan-change-delta__label">{t("approval.recoveryPlanAdded")}</div>
                    {recoveryPlanDelta.added.map((line, index) => (<div className="plan-change-delta__line" key={`added-${index}-${line}`}><span>+</span>{line}</div>))}
                  </div>)}
              </section>)}
            {isRecoveryApproval && reasonOpen && (<dl className="approval-details recovery-details">
                {recovery?.failed_summary && (<div className="recovery-detail-row">
                ...
                {recovery?.diagnosis && (<div className="recovery-detail-row">
                    <dt>{t("approval.recoveryDiagnosisLabel")}</dt>
                    <dd>{recovery.diagnosis}</dd>
                  </div>)}
                {(recovery?.change_rationale || recovery?.review_rationale) && (<div className="recovery-detail-row">
                    <dt>{t("approval.recoveryWhyLabel")}</dt>
                    <dd>{recovery.change_rationale || recovery.review_rationale}</dd>
                  </div>)}
                {recovery?.source_agent && (<div className="recovery-detail-row">
                    <dt>{t("approval.recoverySourceLabel")}</dt>
                    <dd><code>{recovery.source_agent}</code></dd>
                  </div>)}
              </dl>)}
            {!isPlanApproval && !isRecoveryApproval && subject && (<div className="approval-details">
                <pre className="approval-subject">{subject}</pre>
                {reasonOpen && reason && <div className="approval-reason">{reason}</div>}
              </div>)}
            {isPlanApproval && revisionOpen && (<div className="plan-revision">
                <textarea ref={inputRef} className="plan-revision__input" value={revisionText} rows={3} placeholder={t("approval.revisePlanPlaceholder")} onChange={(event) => setRevisionText(event.target.value)} onFocus={() => onRevisionActiveChange?.(true)} onKeyDown={onRevisionKeyDown} disabled={submitting}/>
                {fileMenu.open && (<FileReferenceMenu items={fileMenu.items} activeIndex={fileMenu.active} onPick={pickRevisionFile} onHover={fileMenu.setActive}/>)}
...
export { approvalToolLabel as approvalToolLabel } from "./approval_modal_helpers";
export { requiresFreshHumanApproval as requiresFreshHumanApproval } from "./approval_modal_helpers";
export { APPROVAL_MODE_RANK as APPROVAL_MODE_RANK } from "./approval_modal_helpers";
export { localizeApprovalSubject as localizeApprovalSubject } from "./approval_modal_helpers";
export { localizeApprovalReason as localizeApprovalReason } from "./approval_modal_helpers";
export { localizePlanModeApprovalReason as localizePlanModeApprovalReason } from "./approval_modal_helpers";
export type { DecisionAction as DecisionAction } from "./approval_modal_helpers";
export { RECOVERY_FEEDBACK_MAX as RECOVERY_FEEDBACK_MAX } from "./approval_modal_helpers";
export { recoveryReasonText as recoveryReasonText } from "./approval_modal_helpers";
export { planDelta as planDelta } from "./approval_modal_helpers";
