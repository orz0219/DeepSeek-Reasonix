...
export function StatusBar({ context, usage, balance, running, sessionTurns, sessionTokens, turnTokens, lastTurnOutputTokens, lastTurnModelMs, lastTurnOutputEstimated = false, lastRequestTps, turnCost, cost, currency, modelLabel, labelStyle = "text", items, workspacePath, workspaceName, gitBranch, onConnectRemote, onDisconnectRemote, onManageRemote, onOpenRemote, onOpenRemoteWorkspace, remoteHosts = [], remoteStatuses = {}, jobs = [], onCancelJob, backgroundRuntimes = [], onCancelRuntimeJob, onRevealRuntime, extensionStatuses = [], }: {
    ...
    const tpsLabel = lastRequestTps === undefined
        ? formatTps(lastTurnOutputTokens && lastTurnModelMs ? lastTurnOutputTokens / (lastTurnModelMs / 1000) : null, lastTurnOutputEstimated)
    ...
    const itemRenderers: Record<StatusBarItemId, ReactNode> = {
        model: (<Tooltip label={t("status.modelTitle")}>
        ...
        workspace: workspaceLabel ? (<Tooltip label={workspaceTitle} className="statusbar__metric statusbar__metric--workspace">
        <span className="stat statusbar__workspace">
          <span className="stat__label stat__label--icon" aria-hidden="true"><Folder size={12}/></span>
          <b>{workspaceLabel}</b>
        </span>
      </Tooltip>) : null,
        git_branch: branchLabel ? (<Tooltip label={`${t("status.gitBranchTitle")}: ${branchLabel}`} className="statusbar__metric statusbar__metric--branch">
        <span className="stat statusbar__branch">
          <span className="stat__label stat__label--icon" aria-hidden="true"><GitBranch size={12}/></span>
          <b>{branchLabel}</b>
        </span>
      </Tooltip>) : null,
        cache: (<Tooltip label={cacheTooltip} className="statusbar__metric statusbar__metric--cache">
        ...
        cache_avg: (<Tooltip label={avgCacheTooltip} className="statusbar__metric statusbar__metric--avg">
        ...
        session_tokens: (<Tooltip label={t("status.sessionTokensTitle")} className="statusbar__metric statusbar__metric--tokens">
        ...
        turn_tokens: (<Tooltip label={t("status.turnTokensTitle")} className="statusbar__metric statusbar__metric--turn-tokens">
        ...
        turn_cost: (<Tooltip label={t("status.turnCostTitle")} className="statusbar__metric statusbar__metric--turn-cost">
        ...
        session_turns: (<Tooltip label={t("status.sessionTurnsTitle")} className="statusbar__metric statusbar__metric--turns">
        ...
        turn_tps: (<Tooltip label={t("status.tpsTitle")} className="statusbar__metric statusbar__metric--tps">
        ...
        turn_output_tokens: (<Tooltip label={t("status.outputTokensTitle")} className="statusbar__metric statusbar__metric--output-tokens">
        ...
        turn_cache_tokens: (<Tooltip label={t("status.cacheTokensTitle")} className="statusbar__metric statusbar__metric--cache-tokens">
        ...
        context: (<Tooltip label={t("status.ctxTitle")} className="statusbar__metric statusbar__metric--ctx">
        ...
        compact: (<Tooltip label={t("status.compactTitle")} className="statusbar__metric statusbar__metric--compact">
        ...
        cost: (<Tooltip label={t("status.spendTitle")} className="statusbar__metric statusbar__metric--cost">
        ...
        balance: (<Tooltip label={balanceTitle} className="statusbar__metric statusbar__metric--balance">
...
function ExtensionStatusBarChips({ statuses }: {
    ...
    return (<>
      {statuses.map((status) => {
            ...
            return (<span className="statusbar__item" data-statusbar-item="extension" key={`${status.pluginId}:${status.surfaceId}`}>
            <Tooltip label={<span className="statusbar__tooltip-stack">
...
function JobsStatusBarChip({ jobs, activeJobsRemote, onCancelJob, runtimes, onCancelRuntimeJob, onRevealRuntime, }: {
    jobs: JobView[];
    ...
    const totalActivity = groups.reduce((total, runtime) => total + Math.max(1, runtime.jobs.length), 0);
    useEffect(() => {
        if (totalActivity === 0)
            setOpen(false);
    }, [totalActivity]);
    ...
    const stop = async (tabID: string, jobID: string) => {
        ...
        if (!handler || stopping.has(key))
            return;
        setStopping((current) => new Set(current).add(key));
        try {
            if (tabID && onCancelRuntimeJob)
                await onCancelRuntimeJob(tabID, jobID);
            else if (onCancelJob)
    ...
    return (<span className="statusbar__jobs">
      <button ref={triggerRef} type="button" className="statusbar__jobs-trigger" aria-label={`${t("status.jobsTitle")}: ${t("status.jobs", { n: totalActivity })}`} aria-expanded={open} aria-haspopup="dialog" title={t("status.jobsTitle")} onClick={() => setOpen((value) => !value)}>
      ...
      <AnchoredPopover open={open} anchorRef={triggerRef} onClose={() => setOpen(false)} className="jobs-popover" align="start">
        <section role="dialog" aria-label={t("status.jobsTitle")}>
          ...
          <div className="jobs-popover__list">
            {groups.map((runtime) => (<div className="jobs-popover__runtime" key={runtime.tabId || "active"}>
                {runtime.tabId && (<div className="jobs-popover__runtime-header">
                    <strong>{runtime.title || t("runtime.unknownTask")}</strong>
                    {onRevealRuntime && (<button type="button" className="btn btn--small" onClick={() => void onRevealRuntime(runtime.tabId)}>
                        {t("status.jobOpenTask")}
                      </button>)}
                  </div>)}
                {runtime.jobs.length === 0 && (<div className="jobs-popover__job">
                ...
                return (<div className="jobs-popover__job" key={`${runtime.tabId}:${job.id}`}>
                      <span className="jobs-popover__copy">
                        <strong>{job.label || job.kind}</strong>
                        <small>{job.kind} · {job.status}</small>
                      </span>
                      <button type="button" className="btn btn--small jobs-popover__stop" disabled={pending || !canStop} onClick={() => void stop(runtime.tabId, job.id)}>
...
export { sourceCacheTooltip as sourceCacheTooltip } from "./status_bar_helpers";
export { MetricLabel as MetricLabel } from "./status_bar_helpers";
export { compactPath as compactPath } from "./status_bar_helpers";
export { workspaceTooltip as workspaceTooltip } from "./status_bar_helpers";
