...
import { ChevronDown, ChevronRight, FileText, Folder, FolderTree, FolderX, GitBranch, Maximize2, MessageSquarePlus, Minimize2, RefreshCw, Search, X } from "lucide-react";
import { asArray } from "../lib/array";
import { app } from "../lib/bridge";
import { useT } from "../lib/i18n";
import { initialWorkspaceSplitTreeWidth, resolveWorkspaceSplitTreeWidth, shouldInitializeWorkspaceSplitOnFileSelect, type WorkspaceSplitTreeWidthMode, workspaceSplitCanFit, workspaceSplitTreeWidthFromPointer } from "../lib/workspaceSplit";
...
import { readWorkspaceTreeMemory, rememberWorkspaceTreeOpenDirs, touchWorkspaceTreeVisit, workspaceTreeVisitId } from "../lib/workspaceTreeMemory";
import type { DirEntry, FilePreview, GitCommitView, GitCommitDetailView, RewindResultView, WorkspaceChangeDetailView, WorkspaceChangesView, WireCompletionSummary } from "../lib/types";
import { workspaceGitStatusLabel } from "../lib/workspaceChanges";
import { completionGapLabel, completionPresetLabel, completionReviewLabel, completionVerdictLabel } from "../lib/completionSummaryDisplay";
...
import { WORKSPACE_TREE_MIN_WIDTH, WORKSPACE_TREE_DEFAULT_WIDTH, WORKSPACE_TREE_RAIL_WIDTH, WORKSPACE_PREVIEW_MIN_WIDTH, WORKSPACE_DUAL_PANEL_TARGET_WIDTH, WORKSPACE_CONTEXT_MENU_SELECTION_HEIGHT, WORKSPACE_MAX_PREVIEW_TABS, WorkspaceRevealRequest, WorkspaceFileListRequest, WorkspaceChangeListEntry, WorkspaceChangeListRequest, clampWorkspaceTreeWidth, entryPath, basename, parentPath, parentDirs, topLevelDirPath, renderMediaPreview, shortCwd, formatBytes, formatCommitDate, TreeRow } from "./workspace_panel_helpers";
export function WorkspacePanel({ open, tabId, cwd, maximized, panelWidth, onClose, onToggleMaximized, onPreviewModeChange, onAddToChat, onAddCodeToChat, onOpenInTerminal, onRequestPanelWidth, onFileTreeRefresh, onSessionRevertCommitted, initialViewMode = "files", revealPathRequest, changeRevealRequest, fileListRequest, changeListRequest, showViewTabs = true, workspaceScopeKey: workspaceScopeKeyProp, workspaceMemoryKey: workspaceMemoryKeyProp, workspaceMemoryVisitId: workspaceMemoryVisitIdProp, creationMode = false, completionSummary, }: {
    open: boolean;
    tabId?: string;
    cwd?: string;
    maximized: boolean;
    ...
    const [openDirs, setOpenDirs] = useState<Set<string>>(() => new Set(initialWorkspaceMemory?.openDirs ?? [""]));
    const [revealedRootPaths, setRevealedRootPaths] = useState<Set<string> | null>(() => initialWorkspaceMemory && initialWorkspaceMemory.visitId !== workspaceMemoryVisitId ? new Set() : null);
    ...
    const updateOpenDirs = useCallback((update: (previous: ReadonlySet<string>) => Set<string>) => {
    ...
    useEffect(() => {
        ...
        if (remembered)
            touchWorkspaceTreeVisit(workspaceMemoryKey, workspaceMemoryVisitId);
        else
            rememberWorkspaceTreeOpenDirs(workspaceMemoryKey, nextOpenDirs, workspaceMemoryVisitId);
    }, [workspaceMemoryKey, workspaceMemoryVisitId]);
    const loadDir = useCallback(async (dir: string) => {
        ...
        if (currentWorkspaceScopeKeyRef.current !== requestScopeKey ||
    ...
    const loadChangeDetail = useCallback(async () => {
        ...
        try {
            ...
            if (changeDetailRequestIdRef.current === requestId && currentWorkspaceScopeKeyRef.current === requestScopeKey) {
                setChangeDetail(null);
                setChangeDetailErr(String((err as {
                    message?: unknown;
                })?.message ?? err));
            }
        }
    ...
    useEffect(() => {
        if (!open)
            return;
        if (lastWorkspaceScopeKeyRef.current === workspaceScopeKey)
            return;
    ...
    useEffect(() => {
        ...
        const scopedPathsSettled = scopedFilePaths !== null &&
            scopedFilePaths.length === paths.length &&
            scopedFilePaths.every((path, index) => path === paths[index]);
        if (dismissedFileListRequestIdRef.current === fileListRequest.id)
            return;
        if (lastFileListRequestIdRef.current === fileListRequest.id && viewMode === "files" && scopedPathsSettled)
            return;
    ...
    useEffect(() => {
        ...
        const scopedChangesSettled = scopedChangeRows !== null &&
            scopedChangeRows.length === changes.length &&
            scopedChangeRows.every((change, index) => change.path === changes[index]?.path);
        if (dismissedChangeListRequestIdRef.current === changeListRequest.id)
            return;
        if (lastChangeListRequestIdRef.current === changeListRequest.id && viewMode === "changed" && scopedChangesSettled)
            return;
    ...
    useEffect(() => {
        if (!open || !revealPathRequest)
            return;
        if (dismissedRevealRequestIdRef.current === revealPathRequest.id)
            return;
    ...
    useEffect(() => {
        if (!open || !changeRevealRequest)
            return;
        if (dismissedChangeRevealRequestIdRef.current === changeRevealRequest.id)
            return;
    ...
    const toggleDir = useCallback((dir: string, compactPaths: string[] = [dir]) => {
    ...
    const sessionChanges = useMemo(() => workspaceChanges?.files.filter((c) => c.sources.includes("session")) ?? [], [workspaceChanges]);
    /** Working-tree files that are not already covered by the session section. */
    const gitWorkingChanges = useMemo(() => workspaceChanges?.files.filter((c) => c.sources.includes("git") && !c.sources.includes("session")) ?? [], [workspaceChanges]);
    ...
    const workspaceGitWarning = workspaceChanges && (!workspaceChanges.gitAvailable || workspaceChanges.gitErr?.trim())
        ...
        : null;
    const renderChangeScope = (title: string, changes: typeof sessionChanges) => (<div className="workspace-change-scope">
      ...
      <div className="workspace-change-scope__list">
        {changes.map((change) => {
            ...
            return (<div key={change.path} className="workspace-change-row">
              <button className="workspace-change" type="button" onClick={() => selectFile(change.path)}>
                <FileText size={14}/>
              ...
              {change.canSessionRevert && change.sources.includes("session") && (<button type="button" className="workspace-change__revert" title={t("workspace.revertSessionFile")} onClick={(e) => {
                        e.stopPropagation();
                        void (async () => {
                            ...
                            if (plan?.conflicts?.length) {
                                const ok = window.confirm(t("workspace.revertSessionFileConflict", {
                                    path: change.path,
                            ...
                            const result = await app.CommitWorkspaceFileRevertForTab(workspaceTabId, plan.planId || "", resolution);
                            if (result?.ok) {
    ...
    useEffect(() => {
        ...
        app.SearchFileRefsForTab(workspaceTabId, q).then((results) => {
            if (!cancelled)
                setSearchResults(asArray(results));
        }).catch(() => {
            if (!cancelled)
                setSearchResults(null);
        });
        ...
        const build = (dir: string, depth: number) => {
            ...
            for (const entry of entries) {
                ...
                while (true) {
                    ...
                    if (!children || children.length !== 1 || !children[0]?.isDir)
                        break;
    ...
    useEffect(() => {
        if (!open || !compactProbeKey)
            return;
        compactProbePaths.forEach((path) => {
            if (compactProbeInFlightRef.current.has(path))
                return;
    ...
    const splitPanesFit = useMemo(() => workspaceSplitCanFit({
        panelWidth,
        railWidth: WORKSPACE_TREE_RAIL_WIDTH,
        treeMinWidth: WORKSPACE_TREE_MIN_WIDTH,
        previewMinWidth: WORKSPACE_PREVIEW_MIN_WIDTH,
    ...
    const effectiveTreeWidth = useMemo(() => resolveWorkspaceSplitTreeWidth({
        mode: treeWidthMode,
    ...
    const maxTreeWidthForPanel = useMemo(() => Math.max(WORKSPACE_TREE_MIN_WIDTH, (panelWidth ?? WORKSPACE_DUAL_PANEL_TARGET_WIDTH) - WORKSPACE_TREE_RAIL_WIDTH - WORKSPACE_PREVIEW_MIN_WIDTH), [panelWidth]);
    useEffect(() => {
        ...
        if (!shouldScrollWorkspaceTreeSelection({ selectedPath, pendingRevealPath, actualTreeVisible, selectedIndex }))
            return;
    ...
    }) as CSSProperties, [effectiveTreeWidth]);
    useEffect(() => {
        if (lastPreviewModeActiveRef.current === previewModeActive)
            return;
        lastPreviewModeActiveRef.current = previewModeActive;
        onPreviewModeChange?.(previewModeActive);
    }, [onPreviewModeChange, previewModeActive]);
    useEffect(() => {
        if (open && !treeVisible && !previewVisible)
            onClose();
    ...
    const setSavedTreeWidth = useCallback((width: number) => {
        const next = clampWorkspaceTreeWidth(width, panelWidth);
        setTreeWidth(next);
        setTreeWidthMode("manual");
    }, [panelWidth]);
    const startTreeResize = useCallback((event: ReactPointerEvent<HTMLButtonElement>) => {
        if (!treeVisible)
            return;
    ...
    const resizeTreeWithKeyboard = useCallback((event: KeyboardEvent<HTMLButtonElement>) => {
    ...
    const openSelectionMenu = (event: ReactMouseEvent<HTMLDivElement>) => {
        if (!selectedPath || loadingPreview || preview?.err || preview?.binary || preview?.kind)
            return;
    ...
    const showSelectionToolbar = (event: ReactMouseEvent<HTMLDivElement>) => {
        ...
        if ((event.target as HTMLElement | null)?.closest(".floating-menu"))
            return;
        if (!selectedPath || loadingPreview || preview?.err || preview?.binary || preview?.kind)
            return;
        const text = selectedTextFromPreview();
    ...
    const addTreeFileToChat = async () => {
        ...
        try {
            ...
            if (onAddCodeToChat)
                ...
                onAddToChat?.(formatSelectionReference(target.path, body));
        }
        catch {
            if (currentWorkspaceScopeKeyRef.current !== requestScopeKey)
                return;
    ...
    const renderNormalRow = (row: TreeRow) => {
        ...
        return (<button key={path} className={`workspace-tree__row${active ? " workspace-tree__row--active" : ""}`} data-workspace-path={path} draggable onDragStart={(event) => startTreeDrag(event, path, entry.isDir)} onClick={() => {
                if (entry.isDir) {
            ...
            }} onContextMenu={(event) => openTreeMenu(event, path, entry.isDir)} style={{ paddingLeft: 8 + depth * 14 }}>
        {depth > 0 && (<span className="workspace-tree__guides" aria-hidden="true">
            {Array.from({ length: depth }, (_, index) => (<span className="workspace-tree__guide" key={index} style={{ left: 14 + index * 14 }}/>))}
          </span>)}
        {entry.isDir ? (<ChevronRight size={13} className={`workspace-tree__chev ${isOpen ? "workspace-tree__chev--open" : ""}`} style={{
                    transition: "transform 0.15s ease",
                    transform: isOpen ? "rotate(90deg)" : "rotate(0deg)",
                }}/>) : (<span className="workspace-tree__chev"/>)}
        {entry.isDir ? (<Folder size={14} className="workspace-tree__icon workspace-tree__icon--dir"/>) : (<WorkspaceFileIcon fileName={entry.name}/>)}
    ...
    const renderSearchRow = (row: TreeRow) => {
        ...
        return (<button key={path} className={`workspace-tree__row workspace-tree__row--search${active ? " workspace-tree__row--active" : ""}`} data-workspace-path={path} draggable onDragStart={(event) => startTreeDrag(event, path, entry.isDir)} onClick={() => {
                if (entry.isDir) {
                    toggleDir(path);
                }
                else {
    ...
    const codePreviewActive = Boolean(selectedPath &&
        !changedMode &&
        preview &&
        !loadingPreview &&
        !preview.err &&
    ...
    const openCodeSearch = () => {
        if (!codePreviewActive || !selectedPath)
            return;
        setCodeSearchRequestPath(selectedPath);
        setCodeSearchRequestPending(true);
    };
    return (<aside ref={panelRef} className={`workspace-panel${embeddedDockMode ? " workspace-panel--embedded" : ""}${showTreeRail ? " workspace-panel--with-tree-rail" : ""}${changedMode ? " workspace-panel--detail-only" : ""}${changedMode && !selectedPath ? " workspace-panel--changed-overview" : ""}${previewVisible && actualTreeVisible ? " workspace-panel--split-preview" : ""}${actualTreeVisible ? "" : " workspace-panel--tree-hidden"}${previewVisible ? "" : " workspace-panel--preview-hidden"}${treeResizing ? " workspace-panel--tree-resizing" : ""}`} aria-label={t("workspace.title")} style={panelStyle} onKeyDownCapture={(event) => {
      ...
      {previewVisible && <section className="workspace-preview">
        <header className="workspace-preview__head">
          <div className="workspace-current-file" aria-label={t("workspace.currentFile")}>
            {changedMode && !selectedPath ? (<GitBranch size={15} className="workspace-current-file__icon"/>) : (<FileText size={15} className="workspace-current-file__icon"/>)}
            ...
            <Tooltip label={t("workspace.recentFiles")}>
              <button ref={recentAnchorRef} className={`workspace-current-file__recent${recentOpen ? " workspace-current-file__recent--open" : ""}`} type="button" aria-label={t("workspace.recentFiles")} aria-expanded={recentOpen} onClick={() => setRecentOpen((open) => !open)}>
                <ChevronDown size={13}/>
              </button>
            </Tooltip>
          </div>
          ...
          <div className="workspace-preview__window-actions">
            {codePreviewActive && (<Tooltip label={t("workspace.searchPlaceholder")}>
                <button className="workspace-iconbtn" type="button" aria-label={t("workspace.searchPlaceholder")} aria-keyshortcuts="Control+F Meta+F" onClick={openCodeSearch}>
                  <Search size={15}/>
                </button>
              </Tooltip>)}
            ...
            {selectedPath && (<Tooltip label={t("workspace.closePreview")}>
                <button className="workspace-iconbtn" onClick={closePreviewArea}>
                  <X size={15}/>
                </button>
              </Tooltip>)}
          </div>
          <AnchoredPopover open={recentOpen} anchorRef={recentAnchorRef} onClose={() => setRecentOpen(false)} className="workspace-recent-menu" align="start" offset={6} placement="bottom">
            <div className="workspace-recent-menu__title">{t("workspace.recentFiles")}</div>
            <div className="workspace-recent-menu__list">
              {recentFiles.map((path) => (<button key={path} type="button" className={`workspace-recent-menu__item${path === selectedPath ? " workspace-recent-menu__item--active" : ""}`} onClick={() => {
                    setSelectedPath(path);
                    setRecentOpen(false);
                }}>
                  <FileText size={14}/>
        ...
        <div className="workspace-preview__meta">
          <Tooltip label={cwd}>
            <button className="workspace-crumb" onClick={() => {
                setFilter("");
                showTreeEvenSplit();
          ...
          {pathParts.map((part, index) => {
                ...
                return (<span className="workspace-crumb-group" key={`${part}-${index}`}>
                <span>›</span>
                <Tooltip label={isLast ? (selectedPath ?? undefined) : dir}>
                  <button className={`workspace-crumb${isLast ? " workspace-crumb--current" : ""}`} onClick={() => {
                        if (isLast)
                            return;
                        showTreeEvenSplit();
                        setFilter("");
        ...
        <div className={`workspace-preview__body${codePreviewActive ? " workspace-preview__body--code" : ""}`} ref={previewBodyRef} onContextMenu={openSelectionMenu} onMouseUp={showSelectionToolbar}>
          {viewMode === "changed" && scopedChangeRows ? (<div className="workspace-change-scope">
              <div className="workspace-change-scope__head">
                ...
                <Tooltip label={t("workspace.clearChangeScope")}>
                  <button type="button" aria-label={t("workspace.clearChangeScope")} onClick={() => {
                    dismissedChangeListRequestIdRef.current = lastChangeListRequestIdRef.current;
                    setScopedChangeRows(null);
                    setSelectedPath(null);
                    setExpandedCommit(null);
              ...
              <div className="workspace-change-scope__list">
                {scopedChangeRows.map((change) => {
                    const dir = parentPath(change.path);
                    return (<button key={change.key} className="workspace-change" type="button" onClick={() => {
                            dismissedChangeListRequestIdRef.current = lastChangeListRequestIdRef.current;
                            setScopedChangeRows(null);
            ...
            </div>) : viewMode === "changed" && !selectedPath ? (<div className="workspace-git-history">
              {completionSummary && (<section className={`workspace-note workspace-completion-summary${completionSummaryNeedsAttention(completionSummary) ? " workspace-completion-summary--attention" : ""}`} aria-label={t("completion.panelTitle")}>
                  ...
                  <div className="workspace-completion-summary__details">
                    ...
                    {(completionSummary.gap_kinds?.length ?? 0) > 0 && (<span>{t("completion.gaps", { gaps: completionSummary.gap_kinds!.map((gap) => completionGapLabel(gap, t)).join(t("notice.deliveryRequirementSeparator")) })}</span>)}
                    {completionSummary.constraint_degraded && <span>{t("completion.constraintsLimited")}</span>}
                  </div>
                </section>)}
              {workspaceGitWarning && (<div className="workspace-note workspace-note--warning" role="status">
                  {workspaceGitWarning}
                </div>)}
              {groupedChangesLayout ? (<>
                  ...
                  {!loadingHistory && !hasFileChanges && !workspaceGitWarning && (<div className="workspace-empty">{t("context.noChanges")}</div>)}
                  {loadingHistory ? (<div className="workspace-empty">{t("workspace.loading")}</div>) : (<section className={`workspace-commit-history${commitHistoryOpen ? " workspace-commit-history--open" : ""}`}>
                      <button className="workspace-commit-history__toggle" type="button" aria-expanded={commitHistoryOpen} onClick={() => {
                      ...
                      {commitHistoryOpen && (<div className="workspace-git-history__list">
                          {gitHistory.map((commit) => (<div key={commit.hash} className={`workspace-git-history__item${expandedCommit === commit.hash ? " workspace-git-history__item--expanded" : ""}`}>
                              ...
                              {expandedCommit === commit.hash && (<div className="workspace-git-history__detail">
                                  {loadingCommit ? (<div className="workspace-empty">{t("workspace.loading")}</div>) : commitDetail?.diff ? (<CodeViewer value={cleanGitDiff(commitDetail.diff)} language="diff"/>) : commitDetail?.files ? (<div className="workspace-git-history__files">
                                      {commitDetail.files.map((file) => (<button key={file} className="workspace-git-history__file" onClick={() => selectFile(file)}>
                                          <FileText size={14}/> {file}
                                        </button>))}
                                    </div>) : (<div className="workspace-empty">No details available</div>)}
                                </div>)}
                            </div>))}
                        </div>)}
                    </section>)}
                  ...
                  {gitWorkingChanges.length > 0 && renderChangeScope(t("workspace.workingChanges"), gitWorkingChanges)}
                  {loadingHistory ? (<div className="workspace-empty">{t("workspace.loading")}</div>) : gitHistory.length === 0 && !hasFileChanges ? (<div className="workspace-empty">{workspaceGitWarning ? t("workspace.gitChangesUnknown") : t("workspace.noChanges")}</div>) : (<div className="workspace-git-history__list">
                      {gitHistory.map((commit) => (<div key={commit.hash} className={`workspace-git-history__item${expandedCommit === commit.hash ? " workspace-git-history__item--expanded" : ""}`}>
                          <button className="workspace-git-history__head" onClick={() => void toggleCommit(commit.hash)}>
                            <div className="workspace-git-history__head-top">
                              {expandedCommit === commit.hash ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}
              ...
              <section className="workspace-current-change">
                <header className="workspace-current-change__head">
                  <div>
                    ...
                    {changeDetail?.source && (<span>{t(changeDetail.source === "git" ? "workspace.currentChangesSourceGit" : "workspace.currentChangesSourceSession")}</span>)}
                  </div>
                  {(changeDetail?.added || changeDetail?.removed) ? (<small>
                ...
                <div className="workspace-current-change__body">
                  {loadingChangeDetail ? (<div className="workspace-empty">{t("workspace.loading")}</div>) : changeDetailErr ? (<div className="workspace-empty workspace-empty--error">{t("workspace.changeDetailUnavailable")}: {changeDetailErr}</div>) : changeDetail?.truncated ? (<div className="workspace-empty">{t("workspace.changeDetailTooLarge")}</div>) : changeDetail?.binary ? (<div className="workspace-empty">{t("workspace.binaryChange")}</div>) : changeDetail?.diff ? (changeDetail.diff.includes("@@") ? (<DiffView diff={changeDetail.diff} language={languageFor(selectedPath)}/>) : (<CodeViewer value={changeDetail.diff} language="diff"/>)) : (<div className="workspace-empty">{t("workspace.noCurrentDiff")}</div>)}
                </div>
              </section>
              <section className={`workspace-commit-history${commitHistoryOpen ? " workspace-commit-history--open" : ""}`}>
                ...
                {commitHistoryOpen && (loadingHistory ? (<div className="workspace-empty">{t("workspace.loading")}</div>) : gitHistory.length === 0 ? (<div className="workspace-empty">{t("workspace.noCommitHistory")}</div>) : (<div className="workspace-git-history__list">
                      {gitHistory.map((commit) => (<div key={commit.hash} className={`workspace-git-history__item${expandedCommit === commit.hash ? " workspace-git-history__item--expanded" : ""}`}>
                          <button className="workspace-git-history__head" onClick={() => void toggleCommit(commit.hash)}>
                            <div className="workspace-git-history__head-top">
                              {expandedCommit === commit.hash ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}
                          ...
                          {expandedCommit === commit.hash && (<div className="workspace-git-history__detail">
                              {loadingCommit ? (<div className="workspace-empty">{t("workspace.loading")}</div>) : commitDetail?.diff ? (<CodeViewer value={cleanGitDiff(commitDetail.diff)} language="diff"/>) : (<div className="workspace-empty">{t("workspace.noCommitDetail")}</div>)}
                            </div>)}
                        </div>))}
                    </div>))}
              </section>
            </div>) : !selectedPath ? (<div className="workspace-empty">{t("workspace.pickFile")}</div>) : loadingPreview ? (<div className="workspace-empty">{t("workspace.loading")}</div>) : preview?.err ? (<div className="workspace-empty workspace-empty--error">
              {/no such file|not found|enoent/i.test(preview.err) ? t("workspace.fileDeleted") : preview.err}
            </div>) : preview?.kind ? (renderMediaPreview(preview)) : preview?.binary ? (<div className="workspace-empty">{t("workspace.binary")}</div>) : preview ? (<>
              {preview.truncated && <div className="workspace-note">{t("workspace.truncated")}</div>}
              {isMarkdown ? (<Markdown text={preview.body}/>) : (<CodeViewer value={preview.body || " "} language={languageFor(selectedPath)} sourceSize={preview.size} showLineNumbers searchRequestPending={codeSearchRequestPending && codeSearchRequestPath === selectedPath} onSearchRequestConsumed={consumeCodeSearchRequest}/>)}
            </>) : null}
          {selectionMenu && (<FloatingMenu x={selectionMenu.x} y={selectionMenu.y} estimatedHeight={WORKSPACE_CONTEXT_MENU_SELECTION_HEIGHT}>
              <FloatingMenuItems items={[
      ...
      {showTreeRail && (<section className="workspace-tree-rail" aria-label={actualTreeVisible ? t("workspace.hideTree") : t("workspace.showTree")}>
          <Tooltip label={actualTreeVisible ? t("workspace.hideTree") : t("workspace.showTree")} side="right">
            <button className={`workspace-tree-reveal workspace-iconbtn${actualTreeVisible ? " workspace-iconbtn--on" : ""}`} type="button" aria-label={actualTreeVisible ? t("workspace.hideTree") : t("workspace.showTree")} aria-pressed={actualTreeVisible} onClick={toggleTreeRail}>
              <FolderTree size={15}/>
            </button>
          </Tooltip>
        </section>)}

      {actualTreeVisible && previewVisible && (<button className="workspace-tree-resizer" type="button" role="separator" aria-orientation="vertical" aria-label={t("workspace.resizeTree")} aria-valuemin={WORKSPACE_TREE_MIN_WIDTH} aria-valuemax={maxTreeWidthForPanel} aria-valuenow={effectiveTreeWidth} onPointerDown={startTreeResize} onKeyDown={resizeTreeWithKeyboard} onDoubleClick={() => setSavedTreeWidth(WORKSPACE_TREE_DEFAULT_WIDTH)}/>)}

      <section className="workspace-files">
        {showFileTools && (<div className={`workspace-files__tools${embeddedDockMode ? " workspace-files__tools--embedded" : ""}`}>
            {showViewTabs && (<Tooltip label={previewVisible ? t("workspace.hideTree") : t("workspace.close")}>
                <button className="workspace-iconbtn workspace-iconbtn--on" type="button" aria-label={previewVisible ? t("workspace.hideTree") : t("workspace.close")} onClick={hideTreeOrClosePanel}>
                  {previewVisible ? <FolderX size={15}/> : <X size={15}/>}
                </button>
              </Tooltip>)}
            {showViewTabs && (<div className="workspace-files__tabs" role="tablist" aria-label={t("workspace.viewMode")}>
                <button className={viewMode === "files" ? "workspace-files__tab workspace-files__tab--active" : "workspace-files__tab"} onClick={() => setViewMode("files")}>
                  {t("workspace.filesTab")}
                </button>
                <button className={viewMode === "changed" ? "workspace-files__tab workspace-files__tab--active" : "workspace-files__tab"} onClick={() => {
                    setViewMode("changed");
                }}>
                  <GitBranch size={13}/>
            ...
            <Tooltip label={t("workspace.refreshChanges")}>
              <button className="workspace-iconbtn" type="button" aria-label={t("workspace.refreshChanges")} aria-busy={loadingPreview || loadingHistory} onClick={() => {
            ...
            {workspaceRefresh.watchState !== "active" && (<span className="workspace-watch-status" role="status" title={t(workspaceRefresh.watchState === "degraded" ? "workspace.watchDegraded" : "workspace.watchUnavailable")} aria-label={t(workspaceRefresh.watchState === "degraded" ? "workspace.watchDegraded" : "workspace.watchUnavailable")}>
                •
              </span>)}
          </div>)}
        ...
        {scopedFilePaths && (<div className="workspace-files__scope">
            ...
            <Tooltip label={t("workspace.clearFileScope")}>
              <button type="button" aria-label={t("workspace.clearFileScope")} onClick={() => {
                dismissedFileListRequestIdRef.current = lastFileListRequestIdRef.current;
                setScopedFilePaths(null);
                setFilter("");
            }}>
        ...
        <div className="workspace-tree" ref={treeRef} style={{
            height: "100%",
            overflow: "auto",
            position: "relative",
        }}>
          {treeRows.length > 0 ? (<div ref={virtualizer.containerRef} className="workspace-tree__sizer" style={{
                width: "100%",
                position: "relative",
            }}>
              {virtualTreeItems.map((row) => {
                ...
                if (!item)
                    return null;
                return (<div key={item.key} data-index={row.index} ref={virtualizer.measureElement} style={{
                        position: "absolute",
                        top: 0,
                        left: 0,
                        width: "100%",
      ...
      {treeMenu && (<WorkspaceTreeMenu target={treeMenu} workspaceTabId={workspaceTabId} isScopeCurrent={() => currentWorkspaceScopeKeyRef.current === workspaceScopeKey} onClose={() => setTreeMenu(null)} onOpenInTerminal={onOpenInTerminal} onAddReference={addTreeReferenceToChat} onAddFile={() => void addTreeFileToChat()} onRefreshTree={refreshWorkspaceList}/>)}
    </aside>);
}
export { WORKSPACE_TREE_MIN_WIDTH as WORKSPACE_TREE_MIN_WIDTH } from "./workspace_panel_helpers";
export { WORKSPACE_TREE_DEFAULT_WIDTH as WORKSPACE_TREE_DEFAULT_WIDTH } from "./workspace_panel_helpers";
export { WORKSPACE_TREE_RAIL_WIDTH as WORKSPACE_TREE_RAIL_WIDTH } from "./workspace_panel_helpers";
export { WORKSPACE_PREVIEW_MIN_WIDTH as WORKSPACE_PREVIEW_MIN_WIDTH } from "./workspace_panel_helpers";
export { WORKSPACE_DUAL_PANEL_TARGET_WIDTH as WORKSPACE_DUAL_PANEL_TARGET_WIDTH } from "./workspace_panel_helpers";
export { WORKSPACE_CONTEXT_MENU_SELECTION_HEIGHT as WORKSPACE_CONTEXT_MENU_SELECTION_HEIGHT } from "./workspace_panel_helpers";
export { WORKSPACE_MAX_PREVIEW_TABS as WORKSPACE_MAX_PREVIEW_TABS } from "./workspace_panel_helpers";
export type { WorkspaceRevealRequest as WorkspaceRevealRequest } from "./workspace_panel_helpers";
export type { WorkspaceFileListRequest as WorkspaceFileListRequest } from "./workspace_panel_helpers";
export type { WorkspaceChangeListEntry as WorkspaceChangeListEntry } from "./workspace_panel_helpers";
export type { WorkspaceChangeListRequest as WorkspaceChangeListRequest } from "./workspace_panel_helpers";
export { clampWorkspaceTreeWidth as clampWorkspaceTreeWidth } from "./workspace_panel_helpers";
export { entryPath as entryPath } from "./workspace_panel_helpers";
export { basename as basename } from "./workspace_panel_helpers";
export { parentPath as parentPath } from "./workspace_panel_helpers";
export { parentDirs as parentDirs } from "./workspace_panel_helpers";
export { topLevelDirPath as topLevelDirPath } from "./workspace_panel_helpers";
export { renderMediaPreview as renderMediaPreview } from "./workspace_panel_helpers";
export { shortCwd as shortCwd } from "./workspace_panel_helpers";
export { formatBytes as formatBytes } from "./workspace_panel_helpers";
export { formatCommitDate as formatCommitDate } from "./workspace_panel_helpers";
export type { TreeRow as TreeRow } from "./workspace_panel_helpers";
