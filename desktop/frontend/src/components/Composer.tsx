...
import { commandAvailableAtSlashPosition, commandUsesStructuredInvocation, invocationRequests, replaceInvocationTextRange, serializeInvocationSubmit, typedStructuredInvocationDraft, trimInvocationDraft, type ComposerInvocation, type StructuredInvocationSubmit } from "../lib/invocationDisplay";
import { formatTokens } from "../lib/format";
import type { ControllerLiveStore } from "../lib/useController";
import { clearLayoutSize, saveLayoutSize } from "../lib/layoutPreferences";
import { createRafResizeUpdater } from "../lib/resizeDrag";
import { observeComposerMenuViewport } from "../lib/composerMenuViewport";
import { useToast } from "../lib/toast";
import { type CollaborationMode, type CommandInfo, type ComposerInsertRequest, type ContextInfo, type DirEntry, type EffortInfo, type GoalRuntime, type Mode, type PromptHistoryEntry, type SessionMeta, type SessionReference, type SlashArgItem, type SlashArgsResult, type TokenMode, type ToolApprovalMode, type BalanceInfo } from "../lib/types";
import { formatWorkspaceReference, parseWorkspaceReference, readWorkspaceReferenceDrag, WORKSPACE_REF_DRAG_TYPE } from "../lib/workspaceDrag";
...
import { RichComposerInput, slashQueryAt, type RichComposerChangeOrigin, type RichComposerInputHandle, type RichComposerSelection, type RichSlashQuery } from "./RichComposerInput";
import { VirtualMenu } from "./VirtualMenu";
import { activeFileReferenceToken, dirEntryMenuLabel } from "./FileReferenceMenu";
import { activeRefTokenRe, unescapeRefPath } from "../lib/refToken";
import { ContextMenu, contextMenuPointFromEvent, type ContextMenuItem, type ContextMenuPoint } from "./ContextMenu";
import { formatSelectedTextContext, formatSelectionLabel, languageFor, normalizeSelectedText, selectedTextSnippet, type SelectedTextInsertRequest, type SelectedTextReference } from "../lib/selectedTextContext";
import { formatGoalWorkTime } from "../lib/goalRuntime";
import { WorkspaceReference, composerPickFileEntry, Attachment, AttachmentDedupKey, COMPOSER_MIN_HEIGHT, COMPOSER_RUN_STRIP_RESERVED, COMPOSER_AUTO_RESERVED_HEIGHT, PROMPT_HISTORY_PREFETCH_REMAINING, FILE_REF_SEARCH_CACHE_TTL_MS, PastedBlock, FileRefSearchCacheEntry, ComposerDraft, ComposerEditSnapshot, ComposerEditHistory, WebkitFileEntry, DEFAULT_COMPOSER_DRAFT_KEY, MAX_COMPOSER_EDIT_HISTORY, lineCount, shouldFoldPaste, renderPastedBlock, baseName, attachmentName, attachmentExt, hasImageAttachments, formatAttachmentDisplayReference, sortComposerAttachments, workspaceReferenceKey, activePastChatToken, emptyComposerDraft, cloneComposerDraft, attachmentDedupFromKeys, draftHasAttachmentDedupKey, clipboardFiles, clipboardHasImageHint, isPasteShortcut, composerMaxHeight, composerLogicalHeight, clampComposerHeight, composerAutoInputMaxHeight, loadComposerHeight, fmtElapsed, truncatePreview, fmtSessionTime, pastChatTitle, useTick, PAST_CHATS_MENU_ITEM, limitSessionMessages, formatSessionContext } from "./composer_helpers";
...
export function Composer({ running, collaborationMode, toolApprovalMode, tokenMode, turnPhase, goal, goalStatus, goalRuntime, cwd, modelLabel, imageInputEnabled = true, tabId, effort, onSend, onSteer, onCancel, onCycleMode, onSetMode, onSetCollaborationMode, onSetToolApprovalMode, onToggleYoloApprovalMode, onClearGoal, onPauseGoal, onResumeGoal, onSwitchModel, onSetEffort, onSetTokenMode, insertRequest, selectedTextRequest, disabled, submitDisabled = false, readOnly = false, decisionPending = false, ready, turnStartAt, turnWaitAccumMs = 0, promptWaitStartedAt, turnTokens, turnOutputTokens, turnOutputCharsAtUsage, turnModelActiveAt, turnModelActiveMs = 0, liveStore, turnArgChars = 0, retry, suspendedByDecision = false, pendingApprovalLabel, pendingAsk = false, transientDismissSignal, sessionKey, workspaceScopeKey, fileRefRefreshKey, guidanceConsumedKey, guidanceConsumedText, guidanceQueuePreviewItems, showContextWindowRing = false, heroMode = false, context, turnCost, currency, cacheHitTokens, cacheMissTokens, balance, onInvocationMetadataChange, }: {
    running: boolean;
    ...
    onInvocationMetadataChange?: (metadata: Record<string, {
        kind: "skill" | "subagent";
        color?: string;
    }>) => void;
    ...
    const lastGuidanceConsumedKeyByDraftRef = useRef<Record<string, string | undefined>>(guidanceConsumedKey ? { [draftKey]: guidanceConsumedKey } : {});
    ...
    const restoreComposerDraft = (draft: ComposerDraft) => {
        ...
        setRichSlashQuery(next.invocations.length > 0
    ...
    const composerEditSnapshot = (targetDraftKey: string, selection?: RichComposerSelection): ComposerEditSnapshot => {
    ...
    const composerEditStateMatches = (left: ComposerEditSnapshot, right: ComposerEditSnapshot): boolean => left.text === right.text
    ...
    const syncComposerNativeHistory = (targetDraftKey: string, inputType?: string) => {
        ...
        if (inputType === "historyUndo") {
            history.undoNativeBarrier = Boolean(undoTransaction && !composerEditStateMatches(current, undoTransaction.after));
    ...
    const recordComposerEdit = (targetDraftKey: string, before: ComposerEditSnapshot, after: ComposerEditSnapshot) => {
        if (composerEditStateMatches(before, after))
            return;
        const history = editHistoryForDraft(targetDraftKey);
        const previous = history.undo[history.undo.length - 1];
        const nativeBarrierBefore = Boolean(previous
        ...
        if (history.undo.length > MAX_COMPOSER_EDIT_HISTORY)
            history.undo.shift();
        history.redo = [];
    ...
    const restoreComposerEdit = (targetDraftKey: string, snapshot: ComposerEditSnapshot) => {
        ...
        setComposerSelection(snapshot.selection.start, snapshot.selection.end, snapshot.selection.afterInvocationId);
    };
    const canUndoComposerEdit = (targetDraftKey: string): boolean => {
        const history = editHistoryByDraftRef.current[targetDraftKey];
        if (!history || history.undoNativeBarrier)
            return false;
    ...
    const undoComposerEdit = (targetDraftKey: string): boolean => {
        const history = editHistoryByDraftRef.current[targetDraftKey];
        if (!history || !canUndoComposerEdit(targetDraftKey))
            return false;
    ...
    const canRedoComposerEdit = (targetDraftKey: string): boolean => {
        const history = editHistoryByDraftRef.current[targetDraftKey];
        if (!history || history.redoNativeBarrier)
            return false;
    ...
    const redoComposerEdit = (targetDraftKey: string): boolean => {
        const history = editHistoryByDraftRef.current[targetDraftKey];
        if (!history || !canRedoComposerEdit(targetDraftKey))
            return false;
    ...
    const updatePendingGuidanceForDraft = (targetDraftKey: string, update: (items: PendingGuidance[]) => PendingGuidance[]) => {
        if (targetDraftKey === activeDraftKeyRef.current) {
    ...
    const draftIsSubmitting = (targetDraftKey: string): boolean => targetDraftKey === activeDraftKeyRef.current
        ? submittingRef.current
        : Boolean(draftsBySessionRef.current[targetDraftKey]?.submitting);
    const draftHasPendingPaste = (targetDraftKey: string): boolean => targetDraftKey === activeDraftKeyRef.current
    ...
    const clearNativeClipboardPasteTimer = () => {
        if (nativeClipboardPasteTimerRef.current === null)
            return;
    ...
    useEffect(() => {
        ...
        void app.InboxSnapshot(tabId || "").then((snap) => {
            if (!live)
                return;
            const durable = (snap?.items ?? []).map((it: {
                id: string;
                preview: string;
                state?: string;
                intent?: string;
            ...
            updatePendingGuidanceForDraft(draftKey, () => guidanceQueuePreviewKey
                .split("\n")
                .filter(Boolean)
                .map((text, i) => ({ id: `local-${i}`, text, submitText: text })));
        });
    ...
    useEffect(() => {
        if (guidanceExpanded && pendingGuidance.length <= 2)
            setGuidanceExpanded(false);
    ...
    useEffect(() => {
        onInvocationMetadataChange?.(Object.fromEntries(commands
    ...
    const slashMatches = useMemo(() => slashQuery === null
        ? []
        : sortSlashCommandsForMenu(commands.filter((c) => c.name.toLowerCase().includes(slashQuery))), [slashQuery, commands]);
    const slashCommandAtStart = Boolean(activeSlashQuery
        && invocations.length === 0
        && slashText.slice(0, activeSlashQuery.from).trim() === "");
    const slashCommandDisabled = useCallback((command: CommandInfo) => !commandAvailableAtSlashPosition(command, slashCommandAtStart), [slashCommandAtStart]);
    const slashSelectableIndices = useMemo(() => slashMatches.flatMap((command, index) => slashCommandDisabled(command) ? [] : [index]), [slashCommandDisabled, slashMatches]);
    ...
    useEffect(() => {
        if (prevFileRefScopeRef.current === fileRefScopeKey)
            return;
    ...
    useEffect(() => {
        if (prevFileRefRefreshKeyRef.current === fileRefRefreshKey)
            return;
    ...
    useEffect(() => {
        ...
        if (cached) {
            ...
            if (Date.now() - cached.cachedAt < FILE_REF_SEARCH_CACHE_TTL_MS)
                return;
        }
        else {
            setSearchEntries([]);
    ...
    const atMenuItemKey = useCallback((item: AtMenuItem) => item.kind === "pastChats" ? "past:chats" : (item.entry.isDir ? "d:" : "f:") + (item.entry.path || item.entry.name), []);
    // --- which menu (if any) is open --- (slash command names win; then slash
    // arguments; then @-refs — they're rarely valid at once)
    const menuMode: "slash" | "slasharg" | "at" | "pastChats" | null = directPastChats
        ? "pastChats"
    ...
    const countBase = menuMode === "slash"
        ? slashMatches.length
        : menuMode === "slasharg"
            ? argRes!.items.length
            : menuMode === "at"
    ...
    useEffect(() => {
        if (transientDismissSignal === undefined || transientDismissSignal === lastTransientDismissSignal.current)
            return;
    ...
    const takeSelfDispatchedGuidance = useCallback((text: string, targetDraftKey: string): boolean => {
        ...
        if (selfDispatched.length === 0)
            delete selfDispatchedGuidanceByDraftRef.current[targetDraftKey];
        return true;
    }, []);
    useEffect(() => {
        if (guidanceDraftKey !== draftKey || !guidanceConsumedKey)
            return;
        if (guidanceConsumedKey === lastGuidanceConsumedKeyByDraftRef.current[draftKey])
            return;
        lastGuidanceConsumedKeyByDraftRef.current[draftKey] = guidanceConsumedKey;
        const consumed = (guidanceConsumedText ?? "").trim();
        if (consumed && takeSelfDispatchedGuidance(consumed, draftKey))
            return;
    ...
    useEffect(() => {
        if (menuMode && menuMode !== "pastChats")
            setContentMenuOpen(false);
    }, [menuMode]);
    ...
    const syncPromptHistoryGeneration = () => {
        ...
        if (historyGenerationRef.current === nextGeneration)
            return;
    ...
    const ensurePromptHistoryIndex = async (index: number): Promise<boolean> => {
        if (index < historyEntriesRef.current.length)
            return true;
        ...
        while (index >= historyEntriesRef.current.length) {
            ...
            historyLoadRef.current = null;
            if (loaded === 0)
                return index < historyEntriesRef.current.length;
        }
        return true;
    };
    ...
    const getComposerSelection = () => {
        if (invocationsRef.current.length > 0)
            return richInputRef.current?.getSelection() ?? richSelection;
    ...
    const setTextCaretEnd = (next: string, trackEdit = true) => {
        ...
        if (beforeEdit) {
            recordComposerEdit(targetDraftKey, beforeEdit, composerEditSnapshot(targetDraftKey, { start: next.length, end: next.length }));
        }
    };
    const rememberCaret = () => {
        if (invocationsRef.current.length > 0) {
    ...
    const insertTextAtCaret = (snippet: string) => {
        ...
        recordComposerEdit(targetDraftKey, beforeEdit, composerEditSnapshot(targetDraftKey, { start: pos, end: pos }));
    };
    ...
    const addWorkspaceReference = (ref: WorkspaceReference) => {
        setWorkspaceRefs((prev) => {
            ...
            if (prev.some((item) => workspaceReferenceKey(item) === key))
                return prev;
            const next = [...prev, ref];
    ...
    useEffect(() => {
        if (!insertRequest || insertRequest.id === consumedInsertIdByDraftRef.current[draftKey])
            return;
    ...
    useEffect(() => {
        if (!selectedTextRequest || selectedTextRequest.id === consumedSelectedTextIdByDraftRef.current[draftKey])
            return;
        ...
        if (normalized.truncated)
            showToast(t("composer.selectedTextTruncated"), "warn");
        const path = selectedTextRequest.path;
        const duplicate = selectedTextRefsRef.current.some((reference) => reference.text === normalized.text && (reference.path ?? "") === (path ?? ""));
        if (!duplicate) {
            const next = [
                ...selectedTextRefsRef.current,
                {
    ...
    const attachmentSeenInDraft = (targetDraftKey: string, key: AttachmentDedupKey): boolean => {
        if (targetDraftKey === activeDraftKeyRef.current)
            return attachmentDedupRef.current.seen(key.hash, key.source);
    ...
    const addAttachmentToDraft = (targetDraftKey: string, attachment: Attachment, key: AttachmentDedupKey): boolean => {
        if (targetDraftKey === activeDraftKeyRef.current) {
            if (attachmentDedupRef.current.seen(key.hash, key.source))
                return false;
        ...
        const draft = cloneComposerDraft(draftsBySessionRef.current[targetDraftKey] ?? emptyComposerDraft());
        if (draftHasAttachmentDedupKey(draft, key))
            return false;
    ...
    const addWorkspaceReferenceToDraft = (targetDraftKey: string, ref: WorkspaceReference) => {
        ...
        const draft = cloneComposerDraft(draftsBySessionRef.current[targetDraftKey] ?? emptyComposerDraft());
        const key = workspaceReferenceKey(ref);
        if (draft.workspaceRefs.some((item) => workspaceReferenceKey(item) === key))
            return;
    ...
    const onIntentHoverEnter = useCallback(() => {
        if (!creationChrome || disabled || running)
            return;
        clearHoverTimer(intentHoverTimerRef);
    ...
    const onIntentHoverLeave = useCallback(() => {
        if (!creationChrome)
            return;
        clearHoverTimer(intentHoverTimerRef);
        if (!intentMenuOpen && !intentMenuClosing)
            return;
        intentHoverTimerRef.current = window.setTimeout(() => {
            intentHoverTimerRef.current = null;
            closeIntentMenu();
    ...
    const submit = async () => {
        ...
        if (draftIsSubmitting(submitDraftKey))
            return;
        ...
        if (draftHasPendingPaste(submitDraftKey))
            return;
    ...
    const sendQueuedGuidance = async (item: PendingGuidance, targetDraftKey = activeDraftKeyRef.current, targetTabId = tabId) => {
        if (targetDraftKey !== activeDraftKeyRef.current || disabled || readOnly || guidanceSendingIdRef.current !== null)
            return;
        ...
        try {
            ...
            if (durable && !running)
                return await kickIdleGuidance(app.SetInboxPaused, targetTabId || "", () => setGuidanceRetryNonce((value) => value + 1));
            if (running && durable) {
                ...
                if (receipt?.disposition === "steer_accepted") {
                    ...
                    updatePendingGuidanceForDraft(targetDraftKey, (items) => items.map((queued) => queued.id === item.id
    ...
    const dismissQueuedGuidance = async (item: PendingGuidance) => {
        try {
            ...
            updatePendingGuidanceForDraft(activeDraftKeyRef.current, (items) => items.filter((queued) => queued.id !== item.id));
        }
        catch (error) {
            showToast(formatInboxError(error, locale), "warn");
        }
    ...
    const attachImageFiles = async (files: File[], sourceDraftKey: string) => {
        ...
        for (const file of images) {
            ...
            try {
                ...
                if (attachmentSeenInDraft(sourceDraftKey, key))
                    continue;
    ...
    const attachNativeClipboardImage = async (notifyOnError: boolean, sourceDraftKey: string) => {
        ...
        try {
            ...
            if (notifyOnError)
                showToast(t("composer.pasteImageFailed"), "warn");
        }
        finally {
            updatePendingPasteForDraft(sourceDraftKey, -1);
        }
    ...
    const onPaste = (e: ClipboardEvent<HTMLTextAreaElement | HTMLDivElement>) => {
        ...
        if (shouldFoldPaste(pasted)) {
            ...
            const next = replaceInvocationTextRange(textRef.current, invocationsRef.current, start, end, label, selection.afterInvocationId);
            ...
            // history.
            resetPromptHistoryNavigation();
            const next = replaceInvocationTextRange(textRef.current, invocationsRef.current, start, end, normalizedPasted, selection.afterInvocationId);
            textRef.current = next.text;
            invocationsRef.current = next.invocations;
            setText(next.text);
            setInvocations(next.invocations);
        ...
        recordComposerEdit(sourceDraftKey, beforeEdit, composerEditSnapshot(sourceDraftKey, { start: caret, end: caret }));
    };
    ...
    const replaceInputRange = (value: string, start: number, end: number, targetDraftKey = activeDraftKeyRef.current, afterInvocationId?: string) => {
        if (targetDraftKey === activeDraftKeyRef.current) {
            const current = textRef.current;
            const next = replaceInvocationTextRange(current, invocationsRef.current, start, end, value, afterInvocationId);
            textRef.current = next.text;
            invocationsRef.current = next.invocations;
            setText(next.text);
            setInvocations(next.invocations);
        ...
        const draft = cloneComposerDraft(draftsBySessionRef.current[targetDraftKey] ?? emptyComposerDraft());
        const next = replaceInvocationTextRange(draft.text, draft.invocations, start, end, value, afterInvocationId);
        draft.text = next.text;
        draft.invocations = next.invocations;
        draftsBySessionRef.current[targetDraftKey] = draft;
    };
    const insertPastedText = (pasted: string, start: number, end: number, targetDraftKey = activeDraftKeyRef.current, afterInvocationId?: string) => {
        const normalizedPasted = pasted.replace(/\r\n/g, "\n");
        ...
        if (targetDraftKey !== activeDraftKeyRef.current) {
            ...
            const next = replaceInvocationTextRange(draft.text, draft.invocations, start, end, inserted, afterInvocationId);
            draft.text = next.text;
            draft.invocations = next.invocations;
            draftsBySessionRef.current[targetDraftKey] = draft;
            recordComposerEdit(targetDraftKey, beforeEdit, composerEditSnapshot(targetDraftKey, { start: caret, end: caret }));
        ...
        if (shouldFoldPaste(pasted)) {
            ...
            const next = replaceInvocationTextRange(textRef.current, invocationsRef.current, start, end, label, afterInvocationId);
            pastedBlocksRef.current = [...pastedBlocksRef.current, block];
            setPastedBlocks((prev) => [...prev, block]);
            textRef.current = next.text;
            invocationsRef.current = next.invocations;
            ...
            resetPromptHistoryNavigation();
            const next = replaceInvocationTextRange(textRef.current, invocationsRef.current, start, end, normalizedPasted, afterInvocationId);
            textRef.current = next.text;
            invocationsRef.current = next.invocations;
            setText(next.text);
            setInvocations(next.invocations);
    ...
    const copyComposerSelection = async (cut = false) => {
        ...
        if (cut) {
            ...
            recordComposerEdit(sourceDraftKey, beforeEdit, composerEditSnapshot(sourceDraftKey, { start: selection.from, end: selection.from }));
        }
    ...
    const pasteIntoComposer = async () => {
        ...
        try {
            ...
            insertPastedText(pasted, selection.from, selection.to, sourceDraftKey, selection.afterInvocationId);
        }
        catch {
            if (sourceDraftKey === activeDraftKeyRef.current) {
                focusInputRange(selection.from, selection.to, selection.afterInvocationId);
    ...
    const hasWorkspaceReferenceDrag = (dataTransfer: DataTransfer): boolean => Array.from(dataTransfer.types).includes(WORKSPACE_REF_DRAG_TYPE);
    const hasFileDrag = (dataTransfer: DataTransfer): boolean => Array.from(dataTransfer.items).some((it) => it.kind === "file") || dataTransfer.files.length > 0;
    const fileDragItems = (dataTransfer: DataTransfer): DataTransferItem[] => Array.from(dataTransfer.items).filter((item) => item.kind === "file");
    ...
    const onFileDropCapture = (e: DragEvent<HTMLDivElement>) => {
        if (hasWorkspaceReferenceDrag(e.dataTransfer) || !hasFileDrag(e.dataTransfer))
            return;
        e.preventDefault();
        if (!hasPathlessFileDrop(e.dataTransfer))
            return;
    ...
    const pickCommand = (c: CommandInfo) => {
        ...
        if (!query || slashCommandDisabled(c))
            return;
        if (!commandUsesStructuredInvocation(c)) {
            if (invocationsRef.current.length > 0 && richSlashQuery) {
                ...
                const next = replaceInvocationTextRange(textRef.current, invocationsRef.current, query.from, query.to, `/${c.name} `);
        ...
        const next = replaceInvocationTextRange(textRef.current, invocationsRef.current, query.from, query.to, "");
        textRef.current = next.text;
        invocationsRef.current = [invocation];
        setText(next.text);
        setInvocations([invocation]);
        setRichSlashQuery(null);
        recordComposerEdit(targetDraftKey, beforeEdit, composerEditSnapshot(targetDraftKey, {
            start: query.from,
            end: query.from,
            afterInvocationId: invocation.id,
        }));
        requestActiveDraftFrame(() => richInputRef.current?.setSelectionRange(query.from, query.from, invocation.id));
    };
    ...
    const replacePastedBlockLabel = (block: PastedBlock, replacement: string): number | null => {
        ...
        if (start < 0)
            return null;
        const next = replaceInvocationTextRange(current, invocationsRef.current, start, start + block.label.length, replacement);
        textRef.current = next.text;
        invocationsRef.current = next.invocations;
        setText(next.text);
        setInvocations(next.invocations);
    ...
    const measureTextareaAutoHeight = useCallback(() => {
        ...
        const nextOverflow = scrollHeight > maxHeight + 1;
        if (node && previousHeight !== undefined)
            node.style.height = previousHeight;
        setTextareaAutoHeight((current) => (current === nextHeight ? current : nextHeight));
        setTextareaAutoOverflow((current) => (current === nextOverflow ? current : nextOverflow));
    ...
    const onComposerResizeKeyDown = (e: KeyboardEvent<HTMLButtonElement>) => {
        ...
        if (e.key === "ArrowUp" || e.key === "PageUp")
            ...
            next = COMPOSER_MIN_HEIGHT;
        else if (e.key === "End")
            next = composerMaxHeight();
        if (next === null)
            return;
        e.preventDefault();
    ...
    const openPastChats = useCallback(async (initialQuery = "") => {
        ...
        try {
            ...
            if (cwdRef.current !== snapshotCwd || activeDraftKeyRef.current !== sourceDraftKey)
                return;
    ...
    useEffect(() => {
        if (!pastChatToken || directPastChats || dismissed || running || disabled || readOnly)
            return;
    ...
    const insertContentTrigger = (trigger: "@" | "#" | "/") => {
        ...
        setShowPastChats(false);
        setDismissed(false);
        replaceInputRange(value, selection.from, selection.to, targetDraftKey, selection.afterInvocationId);
    ...
    const openContentMenu = () => {
        ...
        if (profileMenuOpen || profileMenuClosing)
            closeProfileMenu();
        if (moreMenuOpen || moreMenuClosing)
            closeMoreMenu();
        setDirectPastChats(false);
        setShowPastChats(false);
        setDismissed(true);
    ...
    const pickActive = () => {
        if (menuMode === "slash") {
            ...
            if (item && !slashCommandDisabled(item))
                pickCommand(item);
            return;
        }
    ...
    const onKeyDown = (e: KeyboardEvent<HTMLTextAreaElement | HTMLDivElement>) => {
        ...
        if (canUseCurrentPromptHistory()) {
            ...
            void (async () => {
                ...
                const target = historyDirection === "up"
                    ? sourceIndex + 1
                ...
                const next = historyDirection === "up"
                ...
                if (sourceDraftKey === activeDraftKeyRef.current) {
                    ...
                    recordComposerEdit(sourceDraftKey, beforeEdit, composerEditSnapshot(sourceDraftKey, { start: draft.text.length, end: draft.text.length }));
                }
                ...
                if (menuMode === "slash") {
                    if (slashSelectableIndices.length > 0) {
                        setActive((current) => {
                            ...
                            return slashSelectableIndices[(previousPosition + slashSelectableIndices.length) % slashSelectableIndices.length];
                        });
                    }
                }
                else {
    ...
    const onPastChatSearchKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
        const composing = isImeKeyEvent(e.nativeEvent, pastChatSearchComposingRef.current, pastChatSearchLastCompositionEndAt.current);
    ...
    const chooseTaskMode = (nextMode: CollaborationMode) => {
        closeIntentMenu(() => {
            if (nextMode !== collaborationMode)
                onSetCollaborationMode(nextMode);
            requestActiveDraftFrame(focusComposerInput);
        });
    };
    const stopGoalMode = () => {
    ...
    useEffect(() => {
        ...
        if (trackLocalPause)
            localPauseSinceRef.current = Date.now();
    ...
    useEffect(() => {
        if (trackLocalPause) {
            if (localPauseSinceRef.current == null)
                localPauseSinceRef.current = Date.now();
            return;
        }
        if (localPauseSinceRef.current == null)
        ...
        localPauseSinceRef.current = null;
        if (delta > 0)
            setLocalWaitAccumMs((total) => total + delta);
    }, [trackLocalPause]);
    ...
    const subscribeLiveText = useCallback((cb: () => void) => liveStore?.subscribe(tabId, cb) ?? (() => { }), [liveStore, tabId]);
    const liveTextChars = useSyncExternalStore(subscribeLiveText, () => {
        const live = liveStore?.getSnapshot(tabId);
        return live ? live.text.length + live.reasoning.length : 0;
    });
    const liveModelActiveAt = useSyncExternalStore(subscribeLiveText, () => liveStore?.getModelActiveAt?.(tabId));
    ...
    const editMenuShortcut = (key: string) => formatShortcutCombo(shortcutPlatform === "darwin" ? { key, meta: true } : { key, ctrl: true }, shortcutPlatform);
    ...
    return (<div ref={composerWrapRef} className={[
            "composer-wrap",
            decisionPending ? "composer-wrap--decision-pending" : "",
            heroMode ? "composer-wrap--hero" : "",
        ].filter(Boolean).join(" ")} style={{ "--wails-drop-target": "drop" } as CSSProperties} onDropCapture={onFileDropCapture}>
      <input ref={fileInputRef} className="composer-content-file-input" type="file" multiple tabIndex={-1} aria-hidden="true" onChange={(event) => {
      ...
      <AnchoredPopover open={contentMenuOpen && !disabled && !readOnly && !running} anchorRef={contentMenuAnchorRef} onClose={() => setContentMenuOpen(false)} className="composer-access-menu composer-content-menu" align="start">
        <div className="composer-access-menu__section" role="menu" aria-label={t("composer.contentMenuTitle")}>
          ...
          <button type="button" role="menuitem" className="composer-access-menu__item composer-content-menu__item" onClick={() => insertContentTrigger("/")} disabled={text.trim().length > 0} title={text.trim().length > 0 ? t("composer.contentUseCommandsEmptyOnly") : undefined}>
      ...
      {!heroMode && <AnchoredPopover open={intentMenuOpen} closing={intentMenuClosing} anchorRef={intentMenuAnchorRef} onClose={() => closeIntentMenu()} className="composer-access-menu composer-intent-menu" align="start">
        <div className="composer-access-menu__section" role="menu" aria-label={t("composer.intentMenuTitle")} onMouseEnter={creationChrome ? onIntentPopoverEnter : undefined} onMouseLeave={creationChrome ? onIntentHoverLeave : undefined}>
          <div className="composer-access-menu__label">{t("composer.intentMenuTitle")}</div>
          <button type="button" role="menuitemradio" aria-checked={collaborationMode === "normal"} className={`composer-access-menu__item composer-intent-menu__item${collaborationMode === "normal" ? " composer-access-menu__item--active" : ""}`} onClick={() => chooseTaskMode("normal")} disabled={disabled || running}>
            <ArrowRight size={16}/>
            <span className="composer-access-menu__copy">
          ...
          <button type="button" role="menuitemradio" aria-checked={planModeOn} className={`composer-access-menu__item composer-intent-menu__item${planModeOn ? " composer-access-menu__item--active" : ""}`} onClick={() => chooseTaskMode("plan")} disabled={disabled || running}>
            <List size={16}/>
            <span className="composer-access-menu__copy">
          ...
          <button type="button" role="menuitemradio" aria-checked={goalModeOn} className={`composer-access-menu__item composer-intent-menu__item${goalModeOn ? " composer-access-menu__item--active" : ""}`} onClick={() => chooseTaskMode("goal")} disabled={disabled || running} title={activeGoal || undefined}>
            <Target size={16}/>
            <span className="composer-access-menu__copy">
            ...
            {goalModeOn && activeGoal && (<div className="composer-intent-menu__goal-actions">
              <div className="composer-intent-menu__goal-runtime">
                {goalRuntime && (<span className="composer-intent-menu__goal-runtime-line">
                ...
                {goalStatus === "blocked" && !goalRuntime?.stopCause && (<span className="composer-intent-menu__goal-runtime-line composer-intent-menu__goal-runtime-line--blocked">
                    {t("composer.goalBlocked")}
                  </span>)}
                {goalStatus === "blocked" && goalRuntime?.stopCause && (<span className="composer-intent-menu__goal-runtime-line composer-intent-menu__goal-runtime-line--paused">
                    {t("composer.goalPaused")}
                    {goalRuntime.lastReason ? ` — ${goalRuntime.lastReason}` : ""}
                  </span>)}
              </div>
              {goalStatus === "blocked" ? (<button type="button" className="composer-intent-menu__stop" onClick={onResumeGoal} disabled={disabled}>
                  {t("composer.taskModeResumeGoal")}
                </button>) : (<button type="button" className="composer-intent-menu__stop" onClick={onPauseGoal} disabled={disabled || running}>
                  {t("composer.taskModePauseGoal")}
                </button>)}
              <button type="button" className="composer-intent-menu__stop" onClick={stopGoalMode} disabled={disabled || running}>
                {t("composer.taskModeStopGoal")}
              </button>
            </div>)}
        </div>
      </AnchoredPopover>}
      {!heroMode && <AnchoredPopover open={profileMenuOpen} closing={profileMenuClosing} anchorRef={profileMenuAnchorRef} onClose={() => closeProfileMenu()} className="composer-access-menu composer-profile-menu" align="start">
        <div className="composer-access-menu__section" role="menu" aria-label={t("composer.runtimeProfileTitle")} onMouseEnter={creationChrome ? onProfilePopoverEnter : undefined} onMouseLeave={creationChrome ? onProfileHoverLeave : undefined}>
          ...
          {([
            ...
            ] as const).map(([profile, Icon, titleKey, descKey]) => (<button key={profile} type="button" role="menuitemradio" className={`composer-access-menu__item composer-profile-menu__item${tokenMode === profile ? " composer-access-menu__item--active" : ""}`} onClick={() => chooseTokenMode(profile)} disabled={disabled || running} title={t(descKey)} aria-checked={tokenMode === profile}>
      ...
      <AnchoredPopover open={moreMenuOpen && !disabled && !running} closing={moreMenuClosing} anchorRef={moreMenuAnchorRef} onClose={() => closeMoreMenu()} className="composer-access-menu composer-more-menu" align="end">
        {hasEffort && (<div className="composer-access-menu__section">
            ...
            <div className="composer-more-menu__items" role="listbox" aria-label={t("status.effortTitle")}>
              {effortLevels.map((level) => (<button key={level} type="button" role="option" aria-selected={level === currentEffort} className={`composer-more-menu__item${level === currentEffort ? " composer-more-menu__item--active" : ""}`} onClick={() => chooseEffortLevel(level)} disabled={running}>
                  <Gauge size={14}/>
                  <span>{level}</span>
      ...
      {menuMode === "slash" && (<SlashMenu items={slashMatches} activeIndex={active} onPick={pickCommand} onHover={setActive} isDisabled={slashCommandDisabled} disabledReason={t("slash.startOnly")}/>)}
      {menuMode === "slasharg" && argRes && (<ArgMenu items={argRes.items} activeIndex={active} onPick={pickArg} onHover={setActive}/>)}
      {(menuMode === "at" || menuMode === "pastChats") && (showPastChats ? (<div className="slashmenu" role="listbox">
            {loadingPastChats ? (<div className="slashmenu__item slashmenu__item--empty">
                <span className="slashmenu__name">{t("composer.pastChatsLoading")}</span>
              </div>) : pastChats.length === 0 ? (<div className="slashmenu__item slashmenu__item--empty">
                ...
                <div className="slashmenu__item slashmenu__item--search" onMouseDown={(ev) => ev.preventDefault()}>
                  ...
                  <input className="slashmenu__search" type="text" placeholder={t("composer.pastChatsSearch")} value={pastChatQuery} 
            ...
            autoFocus={!directPastChats} onChange={(ev) => {
                ...
                {filteredPastChats.length === 0 ? (<div className="slashmenu__item slashmenu__item--empty">
                    <span className="slashmenu__name">{t("composer.pastChatsNoMatches")}</span>
                  </div>) : (filteredPastChats.map((session, i) => {
                    ...
                    const tooltipLabel = turnsLabel || ts || preview || pathText ? (<div className="past-chat-hover">
                          <div className="past-chat-hover__title">{pastChatTitle(session)}</div>
                          {preview && <div className="past-chat-hover__preview">{preview}</div>}
                          {(turnsLabel || ts) && (<div className="past-chat-hover__meta">
                              {turnsLabel && <span>{turnsLabel}</span>}
                              {ts && <span>· {fmtSessionTime(ts)}</span>}
                            </div>)}
                          {pathText && <div className="past-chat-hover__path">{pathText}</div>}
                        </div>) : null;
                    return (<Tooltip key={session.path} block label={tooltipLabel}>
                        <button className={`slashmenu__item ${i === active ? "slashmenu__item--active" : ""}`} onMouseDown={(ev) => {
                            ev.preventDefault();
                            pickSession(session);
            ...
            <button className="slashmenu__item slashmenu__item--back" onMouseDown={(ev) => {
                ev.preventDefault();
                if (menuMode === "pastChats")
                    dismissDirectPastChats();
                else {
          ...
          </div>) : menuMode === "at" ? (<VirtualMenu items={atMenuItems} activeIndex={active} itemKey={atMenuItemKey} renderItem={(it, i) => it.kind === "pastChats" ? (<button className={`slashmenu__item${i === active ? " slashmenu__item--active" : ""}`} onMouseDown={(ev) => {
                    ev.preventDefault();
                    void openPastChats();
                }} onMouseMove={() => setActive(i)}>
                  <MessageSquare size={13} className="filemenu__icon"/>
                  <span className="slashmenu__name">{PAST_CHATS_MENU_ITEM}</span>
                </button>) : (<button role="option" aria-selected={i === active} className={`slashmenu__item ${i === active ? "slashmenu__item--active" : ""}`} onMouseDown={(ev) => {
                    ev.preventDefault();
                    pickEntry(it.entry);
                }} onMouseMove={() => setActive(i)}>
                  {it.entry.isDir ? (<Folder size={13} className="filemenu__icon filemenu__icon--dir"/>) : (<FileText size={13} className="filemenu__icon"/>)}
                  <span className="slashmenu__name slashmenu__name--file">
                    {dirEntryMenuLabel(it.entry)}
                    {it.entry.isDir ? "/" : ""}
                  </span>
                </button>)}/>) : null)}
      {pendingGuidance.length > 0 && (<Suspense fallback={null}>
          <ComposerGuidanceShelf recovery={pendingGuidance[0]?.paused ? {
                draftKey,
                tabId: tabId || "",
                count: pendingGuidance[0].recoveredCount || pendingGuidance.length,
                recovered: Boolean(pendingGuidance[0].recoveredCount),
            } : null} recoveryDisabled={Boolean(disabled || readOnly)} items={pendingGuidance} expanded={guidanceExpanded} running={running} disabled={Boolean(disabled)} readOnly={readOnly} sendingId={guidanceSendingId} onReview={() => setGuidanceExpanded(true)} onRecoveryResumed={() => setGuidanceRetryNonce((value) => value + 1)} onRecoveryError={(error) => showToast(formatInboxError(error, locale), "warn")} onToggleExpanded={() => setGuidanceExpanded((value) => !value)} onSend={(item) => void sendQueuedGuidance(item)} onDismiss={(item) => void dismissQueuedGuidance(item)}/>
        </Suspense>)}
      {(attachments.length > 0 || workspaceRefs.length > 0 || sessionRefs.length > 0 || selectedTextRefs.length > 0) && (<div className="composer-context" aria-label={t("composer.contextItems")}>
          {sortComposerAttachments(attachments).map((a) => {
                ...
                return (<ComposerContextCard key={a.path} variant="attachment" tooltipLabel={a.previewUrl ? `${t("imageViewer.clickToPreview")} — ${a.path}` : a.path} removeLabel={t("composer.removeImage")} onRemove={() => removeAttachment(a.path)} previewUrl={a.previewUrl} onImageClick={a.previewUrl ? () => openComposerImageViewer(a.previewUrl!, attachmentName(a)) : undefined} imageOnly={imageOnly} name={attachmentName(a)} meta={attachmentExt(attachmentName(a)) || t("msg.fileAttachment")}/>);
            })}
          {workspaceRefs.map((ref) => (<ComposerContextCard key={workspaceReferenceKey(ref)} variant="workspace" tooltipLabel={ref.displayPath ? formatWorkspaceReference(ref.displayPath, ref.isDir) : formatWorkspaceReference(ref.path, ref.isDir)} removeLabel={t("composer.removeReference")} onRemove={() => removeWorkspaceReference(ref)} folder={Boolean(ref.isDir)} label={ref.isDir ? `${baseName(ref.displayPath || ref.path)}/` : baseName(ref.displayPath || ref.path)}/>))}
          {sessionRefs.map((ref) => (<div className="composer-context__item composer-context__item--session" key={ref.path}>
          ...
          {selectedTextRefs.map((reference) => (<ComposerContextCard key={reference.id} variant="selection" tooltipLabel={reference.path
                    ? <CodeViewer value={reference.text} language={languageFor(reference.path)} maxHeight={240}/>
                    : <Markdown text={reference.text}/>} removeLabel={t("composer.removeSelectedText")} onRemove={() => {
                    const next = selectedTextRefsRef.current.filter((item) => item.id !== reference.id);
                    selectedTextRefsRef.current = next;
                    setSelectedTextRefs(next);
                    requestActiveDraftFrame(focusComposerInput);
                }} name={reference.path ? reference.path.split("/").filter(Boolean).pop() ?? reference.path : selectedTextSnippet(reference.text)} meta={reference.path ? t("composer.selectedCode") : t("composer.selectedText")} icon={reference.path ? <FileText size={20}/> : <MessageSquare size={20}/>}/>))}
        </div>)}
      <ImageViewer open={imageViewer.open} imageUrl={imageViewer.url} imageName={imageViewer.name} onClose={closeComposerImageViewer}/>
      {activePastedBlocks.length > 0 && (<div className="composer__pasted">
          {activePastedBlocks.map((block) => {
                const open = openPastedLabels.includes(block.label);
                return (<div className="composer__pasted-block" key={block.label}>
      ...
      <div className={`composer-card${composerHeight !== null || composerResizing ? " composer-card--resized" : ""}${composerAutoExpanded ? " composer-card--autosized" : ""}${composerResizing ? " composer-card--resizing" : ""}${running ? (waitingPrompt ? " composer-card--waiting" : " composer-card--running") : ""}`} ref={composerCardRef} style={composerCardStyle}>
        <button className="composer-resize-handle" type="button" role="separator" aria-orientation="horizontal" aria-label={t("composer.resize")} aria-valuemin={COMPOSER_MIN_HEIGHT} aria-valuemax={composerMaxHeight()} aria-valuenow={composerResizeValue} title={t("composer.resize")} onPointerDown={onComposerResizeStart} onKeyDown={onComposerResizeKeyDown} onDoubleClick={resetComposerHeight}/>
        {runStateText && (<div className={`composer-run-strip${waitingPrompt ? " composer-run-strip--waiting" : ""}`}>
            ...
            {runTicker ? (<>
                ...
                {runTicker.suffix && (<Tooltip label={t("composer.runStripEstimateHint")}>
        ...
        <div className={`composer${invocations.length > 0 ? " composer--has-invocation" : ""}${dragOver ? " composer--dragover" : ""}${disabled || readOnly ? " composer--disabled" : ""}${shellModeActive ? " composer--shell" : ""}`} onDrop={onDrop} onDragOver={onDragOver} onDragLeave={onDragLeave}>
          <div className="composer__input-row">
            ...
            <div className="composer__content" onMouseDown={focusComposerFromContentBlank}>
              {invocations.length > 0 ? (<RichComposerInput ref={richInputRef} text={text} invocations={invocations} placeholder={composerPlaceholder} disabled={disabled || readOnly} style={textareaStyle} onChange={(nextText, nextInvocations, origin: RichComposerChangeOrigin) => {
                const targetDraftKey = activeDraftKeyRef.current;
                ...
                if (beforeEdit) {
                    recordComposerEdit(targetDraftKey, beforeEdit, composerEditSnapshot(targetDraftKey, origin.afterSelection));
                }
                else {
                ...
                lastSelectionRef.current = { start: selection.start, end: selection.end };
            }} onKeyDown={onKeyDown} onContextMenu={openInputMenu} onPaste={onPaste} onCompositionStart={() => {
                composingRef.current = true;
            }} onCompositionEnd={() => {
                composingRef.current = false;
                lastCompositionEndAt.current = Date.now();
            }}/>) : (<textarea id="composer-input" ref={taRef} className="composer__input" aria-label={t("composer.placeholder")} spellCheck={false} autoCorrect="off" autoCapitalize="off" value={text} onInputCapture={(e) => {
                ...
                if (composerPrompt)
                    setComposerPrompt(null);
            }} onSelect={rememberCaret} onClick={rememberCaret} onKeyUp={rememberCaret} onFocus={rememberCaret} onContextMenu={openInputMenu} onPaste={onPaste} onKeyDown={onKeyDown} onCompositionStart={() => {
                composingRef.current = true;
            }} onCompositionEnd={() => {
                composingRef.current = false;
                lastCompositionEndAt.current = Date.now();
            }} style={textareaStyle} placeholder={composerPlaceholder} rows={1} disabled={disabled || readOnly}/>)}
            </div>
            {composerPrompt && (<span className="composer__prompt" role="status">
                {composerPrompt}
              </span>)}
            {running && (<Tooltip label={t("composer.stop")}>
                <button className="composer__btn composer__btn--stop" type="button" onClick={handleCancel} aria-label={t("composer.stop")}>
                  <Square size={12} fill="currentColor"/>
                </button>
              </Tooltip>)}
            <Tooltip label={submitTooltip}>
              <button className={`composer__btn composer__btn--send${running ? " composer__btn--steer" : ""}`} onClick={submit} disabled={submitBlocked} aria-label={submitTooltip}>
                {running ? <CornerDownRight size={16}/> : <ArrowUp size={16}/>}
              </button>
            </Tooltip>
          </div>
        </div>
        <ContextMenu open={inputMenuPoint !== null} point={inputMenuPoint} items={inputMenuItems} className="context-menu--composer-input" minWidth={64} ariaLabel={t("composer.inputActions")} onClose={() => setInputMenuPoint(null)}/>
        <div className={composerMetaClass}>
          <div className="composer-meta__params">
            {!heroMode && (<div className="composer-meta__control composer-meta__control--content">
                <Tooltip label={t("composer.contentMenuTitle")} disabled={contentMenuOpen}>
                  <button ref={contentMenuAnchorRef} type="button" className={`composer-content-trigger${contentMenuOpen ? " composer-content-trigger--open" : ""}`} onClick={() => (contentMenuOpen ? setContentMenuOpen(false) : openContentMenu())} disabled={disabled || readOnly || running} aria-haspopup="menu" aria-expanded={contentMenuOpen} aria-label={t("composer.contentMenuTitle")}>
                    <Plus size={17} strokeWidth={1.8} aria-hidden="true"/>
                  </button>
                </Tooltip>
              </div>)}
            {!heroMode && (<div className="composer-meta__control composer-meta__control--intent">
                <Tooltip label={taskModeTooltipLabel} disabled={intentMenuOpen || intentMenuClosing || creationChrome}>
                  <button ref={intentMenuAnchorRef} type="button" className={`composer-task-mode-trigger${intentMenuOpen || intentMenuClosing ? " composer-task-mode-trigger--open" : ""}`} onClick={() => (intentMenuOpen || intentMenuClosing ? closeIntentMenu() : openIntentMenu())} onMouseEnter={creationChrome ? onIntentHoverEnter : undefined} onMouseLeave={creationChrome ? onIntentHoverLeave : undefined} disabled={disabled || running} aria-haspopup="menu" aria-expanded={intentMenuOpen && !intentMenuClosing} aria-label={taskModeTriggerLabel} title={intentMenuOpen || intentMenuClosing || creationChrome ? undefined : taskModeTriggerLabel}>
            ...
            {!heroMode && (<div className="composer-meta__control composer-meta__control--profile">
                <Tooltip label={runtimeProfileTooltipLabel} disabled={profileMenuOpen || profileMenuClosing || creationChrome}>
                  <button ref={profileMenuAnchorRef} type="button" data-profile={tokenMode} className={`composer-profile-trigger${profileMenuOpen || profileMenuClosing ? " composer-profile-trigger--open" : ""}`} onClick={() => (profileMenuOpen || profileMenuClosing ? closeProfileMenu() : openProfileMenu())} onMouseEnter={creationChrome ? onProfileHoverEnter : undefined} onMouseLeave={creationChrome ? onProfileHoverLeave : undefined} disabled={disabled || running} aria-haspopup="menu" aria-expanded={profileMenuOpen && !profileMenuClosing} aria-label={runtimeProfileTriggerLabel} title={profileMenuOpen || profileMenuClosing || creationChrome ? undefined : runtimeProfileTriggerLabel}>
            ...
            {!heroMode && (<div className="composer-meta__control composer-meta__control--approval">
                ...
                <div className="composer-modebar composer-modebar--approval" data-mode={toolApprovalMode} title={t("composer.accessMenuTitle", { shortcut: yoloComboLabel })}>
                  <span className="composer-modebar__thumb" aria-hidden="true"/>
                  <button type="button" className={`composer-modebar__item composer-modebar__item--ask${toolApprovalMode === "ask" ? " composer-modebar__item--active" : ""}`} onClick={() => chooseApprovalMode("ask")} disabled={approvalBarDisabled} aria-pressed={toolApprovalMode === "ask"} title={t("composer.accessAskTitle")}>
                    <Shield size={14}/>
                    <span>{t("composer.modeAsk")}</span>
                  </button>
                  <button type="button" className={`composer-modebar__item composer-modebar__item--auto${toolApprovalMode === "auto" ? " composer-modebar__item--active" : ""}`} onClick={() => chooseApprovalMode("auto")} disabled={approvalBarDisabled} aria-pressed={toolApprovalMode === "auto"} title={t("composer.accessAutoTitle")}>
                    <ShieldCheck size={14}/>
                    <span>{t("composer.modeNormal")}</span>
                  </button>
                  <button type="button" className={`composer-modebar__item composer-modebar__item--yolo${toolApprovalMode === "yolo" ? " composer-modebar__item--active" : ""}`} onClick={() => chooseApprovalMode("yolo")} disabled={approvalBarDisabled} aria-pressed={toolApprovalMode === "yolo"} title={t("composer.accessYoloTitle", { shortcut: yoloComboLabel })}>
                    <ShieldAlert size={14}/>
        ...
        */}
            ...
            {!heroMode && hasEffort && (<div className="composer-meta__control composer-meta__control--effort">
                <EffortSwitcher effort={effort} disabled={running} onPick={onSetEffort}/>
              </div>)}
            {!heroMode && hasEffort && (<div className="composer-meta__control composer-meta__control--more">
                <Tooltip label={compactEffortTitle} disabled={moreMenuOpen || moreMenuClosing}>
                  <button ref={moreMenuAnchorRef} type="button" className={`composer-more-trigger composer-more-trigger--effort${currentEffort !== "auto" ? " composer-more-trigger--explicit" : ""}${moreMenuOpen || moreMenuClosing ? " composer-more-trigger--open" : ""}`} onClick={() => (moreMenuOpen || moreMenuClosing ? closeMoreMenu() : openMoreMenu())} disabled={disabled || running} aria-haspopup="menu" aria-expanded={moreMenuOpen && !moreMenuClosing} aria-label={compactEffortTitle} title={moreMenuOpen || moreMenuClosing ? undefined : compactEffortTitle}>
                    <Gauge size={14}/>
...
export type { WorkspaceReference as WorkspaceReference } from "./composer_helpers";
export { composerPickFileEntry as composerPickFileEntry } from "./composer_helpers";
export type { Attachment as Attachment } from "./composer_helpers";
export type { AttachmentDedupKey as AttachmentDedupKey } from "./composer_helpers";
export { COMPOSER_MIN_HEIGHT as COMPOSER_MIN_HEIGHT } from "./composer_helpers";
export { COMPOSER_RUN_STRIP_RESERVED as COMPOSER_RUN_STRIP_RESERVED } from "./composer_helpers";
export { COMPOSER_AUTO_RESERVED_HEIGHT as COMPOSER_AUTO_RESERVED_HEIGHT } from "./composer_helpers";
export { PROMPT_HISTORY_PREFETCH_REMAINING as PROMPT_HISTORY_PREFETCH_REMAINING } from "./composer_helpers";
export { FILE_REF_SEARCH_CACHE_TTL_MS as FILE_REF_SEARCH_CACHE_TTL_MS } from "./composer_helpers";
export type { PastedBlock as PastedBlock } from "./composer_helpers";
export type { FileRefSearchCacheEntry as FileRefSearchCacheEntry } from "./composer_helpers";
export type { ComposerDraft as ComposerDraft } from "./composer_helpers";
export type { ComposerEditSnapshot as ComposerEditSnapshot } from "./composer_helpers";
export type { ComposerEditHistory as ComposerEditHistory } from "./composer_helpers";
export type { WebkitFileEntry as WebkitFileEntry } from "./composer_helpers";
export { DEFAULT_COMPOSER_DRAFT_KEY as DEFAULT_COMPOSER_DRAFT_KEY } from "./composer_helpers";
export { MAX_COMPOSER_EDIT_HISTORY as MAX_COMPOSER_EDIT_HISTORY } from "./composer_helpers";
export { lineCount as lineCount } from "./composer_helpers";
export { shouldFoldPaste as shouldFoldPaste } from "./composer_helpers";
export { renderPastedBlock as renderPastedBlock } from "./composer_helpers";
export { baseName as baseName } from "./composer_helpers";
export { attachmentName as attachmentName } from "./composer_helpers";
export { attachmentExt as attachmentExt } from "./composer_helpers";
export { hasImageAttachments as hasImageAttachments } from "./composer_helpers";
export { formatAttachmentDisplayReference as formatAttachmentDisplayReference } from "./composer_helpers";
export { sortComposerAttachments as sortComposerAttachments } from "./composer_helpers";
export { workspaceReferenceKey as workspaceReferenceKey } from "./composer_helpers";
export { activePastChatToken as activePastChatToken } from "./composer_helpers";
export { emptyComposerDraft as emptyComposerDraft } from "./composer_helpers";
export { cloneComposerDraft as cloneComposerDraft } from "./composer_helpers";
export { attachmentDedupFromKeys as attachmentDedupFromKeys } from "./composer_helpers";
export { draftHasAttachmentDedupKey as draftHasAttachmentDedupKey } from "./composer_helpers";
export { clipboardFiles as clipboardFiles } from "./composer_helpers";
export { clipboardHasImageHint as clipboardHasImageHint } from "./composer_helpers";
export { isPasteShortcut as isPasteShortcut } from "./composer_helpers";
export { composerMaxHeight as composerMaxHeight } from "./composer_helpers";
export { composerLogicalHeight as composerLogicalHeight } from "./composer_helpers";
export { clampComposerHeight as clampComposerHeight } from "./composer_helpers";
export { composerAutoInputMaxHeight as composerAutoInputMaxHeight } from "./composer_helpers";
export { loadComposerHeight as loadComposerHeight } from "./composer_helpers";
export { fmtElapsed as fmtElapsed } from "./composer_helpers";
export { truncatePreview as truncatePreview } from "./composer_helpers";
export { fmtSessionTime as fmtSessionTime } from "./composer_helpers";
export { pastChatTitle as pastChatTitle } from "./composer_helpers";
export { useTick as useTick } from "./composer_helpers";
export { PAST_CHATS_MENU_ITEM as PAST_CHATS_MENU_ITEM } from "./composer_helpers";
export { limitSessionMessages as limitSessionMessages } from "./composer_helpers";
export { formatSessionContext as formatSessionContext } from "./composer_helpers";
