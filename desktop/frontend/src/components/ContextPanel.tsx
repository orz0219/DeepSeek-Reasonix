...
import type { ContextPanelInfo, UsageSourceStats, WireUsage } from "../lib/types";
import { formatMetricTokens, formatCacheHitRate, contextUsageRefreshKey, cacheHitTone, formatSharePercent, contextCostDisplay, contextSessionCache, ContextPanelProps, fmtDuration, fmtUsageCacheRate, MetricTone, UsageAnalysisView, ContextWindowStatus, ContextBreakdown, nonNegativeTokenCount } from "./context_panel_helpers";
/** Prefer Context* (latest attempt) over billable aggregates for turn panels. */
export function liveTurnUsageBreakdown(usage?: WireUsage | null, info?: Pick<ContextPanelInfo, "promptTokens" | "completionTokens" | "reasoningTokens"> | null): {
    promptTokens: number;
    completionTokens: number;
    reasoningTokens: number;
} {
    if (usage) {
        const hasContext = (usage.contextPromptTokens ?? 0) > 0 || (usage.contextCompletionTokens ?? 0) > 0;
        if (hasContext) {
            return {
...
export function contextBreakdown(usedTokens: number, windowTokens: number, promptTokens: number, completionTokens: number, reasoningTokens: number): ContextBreakdown {
...
export function contextWindowStatus(rawUsagePct: number, compactPct: number): ContextWindowStatus {
    if (rawUsagePct > 100)
        return { tone: "warn", key: "context.windowStatusOverLimit" };
    const usagePct = Math.min(100, Math.max(0, rawUsagePct));
    if (usagePct >= 90)
        return { tone: "warn", key: "context.windowStatusNearLimit" };
    if (compactPct > 0 && usagePct >= compactPct)
        return { tone: "warn", key: "context.windowStatusPastCompact" };
    if (compactPct > 0 && usagePct >= Math.max(0, compactPct - 10))
        return { tone: "notice", key: "context.windowStatusWatch" };
...
export function ContextPanel({ tabId, context, usage, sessionTokens, sessionCost, sessionCurrency, turnTokens, turnCost, balance, sessionGen, refreshKey, usageSeq, }: ContextPanelProps) {
    ...
    const renderSourceRow = (row: ContextSourceRow) => {
        ...
        return (<div className="context-panel__source-row" key={row.source}>
        ...
        <details className="context-panel__source-details">
          ...
          <div className="context-panel__source-details-body">
            <SourceSplitBar label={`${t("context.sourceInput")}/${t("context.sourceOutput")}`} segments={[
            ...
            {cacheReported ? (<SourceSplitBar label={`${t("context.sourceCacheHit")}/${t("context.sourceCacheMiss")}`} segments={[
                ...
                ]} compact/>) : (<SourceSplitBar label={`${t("context.sourceCacheHit")}/${t("context.sourceCacheMiss")}`} segments={[]} compact/>)}
        ...
        <section className="context-panel__overview">
          ...
          <section className="context-panel__section context-panel__analysis">
            <SectionHeading title={t("context.usageAnalysis")}>
              {showSourceUsageRows && (<div className="context-panel__view-switch" role="tablist" aria-label={t("context.usageAnalysisView")}>
                  <button type="button" className={`context-panel__view-tab${activeAnalysisView === "source" ? " context-panel__view-tab--active" : ""}`} role="tab" aria-selected={activeAnalysisView === "source"} onClick={() => setAnalysisView("source")}>
                    {t("context.usageAnalysisSource")}
                  </button>
                  <button type="button" className={`context-panel__view-tab${activeAnalysisView === "type" ? " context-panel__view-tab--active" : ""}`} role="tab" aria-selected={activeAnalysisView === "type"} onClick={() => setAnalysisView("type")}>
                    {t("context.usageAnalysisType")}
                  </button>
                </div>)}
            </SectionHeading>
            {activeAnalysisView === "source" ? (<div className="context-panel__source-list" aria-label={t("context.sourceBreakdown")} role="tabpanel">
                ...
                if (sharePct <= 0)
                    return null;
                return (<span className={`context-panel__source-share context-panel__source-tone--${sourceTone(row.source)}`} key={row.source} style={{ width: `${sharePct}%` }}/>);
            })}
                  </div>
                  <div className="context-panel__source-legend">
                    {sourceUsageRows.map((row) => {
                return (<span key={row.source}>
                          <i className={`context-panel__source-dot context-panel__source-tone--${sourceTone(row.source)}`} aria-hidden="true"/>
                ...
                {hiddenSourceRows.length > 0 && (<details className="context-panel__source-more">
              ...
              </div>) : (<div className="context-panel__type-panel" aria-label={t("context.tokenBreakdown")} role="tabpanel">
                <div className="context-panel__type-overview">
                  ...
                  <div className="context-panel__type-sharebar" aria-hidden="true">
                    {tokenTypeRows.map((row) => row.value > 0 ? (<span className={`context-panel__type-share context-panel__type-share--${row.key}`} key={row.key} style={{ width: `${(row.value / Math.max(1, tokenCompositionTotal)) * 100}%` }}/>) : null)}
                  </div>
                  <div className="context-panel__type-legend">
                    {tokenTypeRows.map((row) => (<span key={row.key}>
                ...
                <details className="context-panel__breakdown-details">
                  ...
                  <div className="context-panel__breakdown">
                    {tokenTypeRows.map((row) => (<TokenLegend key={row.key} label={row.label} value={row.value} color={row.key}/>))}
                  </div>
                </details>
              </div>)}
          </section>
...
function SectionHeading({ title, meta, children }: {
    title: string;
    meta?: string;
    children?: ReactNode;
}) {
    return (<header className="context-panel__section-head">
      <h3>{title}</h3>
      {meta && <span>{meta}</span>}
      {children}
    </header>);
}
function TokenLegend({ label, value, color }: {
    label: string;
    value: number;
    color: string;
}) {
    return (<div className="context-panel__legend-row">
      <span className={`context-panel__legend-dot context-panel__legend-dot--${color}`}/>
      <span>{label}</span>
      <strong>{value.toLocaleString()}</strong>
    </div>);
}
function MiniStat({ label, value, title, tone, wide }: {
    label: string;
    value: string;
    title?: string;
    tone?: MetricTone;
    ...
    return (<div className={`context-panel__mini-stat${toneClass}${wideClass}`} aria-label={exactTitle ? `${label}: ${exactTitle}` : undefined}>
      <span>{label}</span>
      <strong title={exactTitle}>{value}</strong>
    </div>);
}
function MetricCard({ label, value, valueTitle, tone, wide }: {
    label: string;
    value: string;
    valueTitle?: string;
    ...
    return (<div className={`context-panel__metric${toneClass}${wideClass}`} aria-label={exactTitle ? `${label}: ${exactTitle}` : undefined}>
      <span>{label}</span>
      <strong title={exactTitle}>{value}</strong>
    </div>);
}
function SourceMetric({ label, value, title }: {
    label: string;
    value: string;
    title?: string;
}) {
    const exactTitle = title && title !== value ? title : undefined;
    return (<div className="context-panel__source-metric" aria-label={exactTitle ? `${label}: ${exactTitle}` : undefined}>
      <span>{label}</span>
      <strong title={exactTitle}>{value}</strong>
    </div>);
}
function SourceSplitBar({ label, segments, compact }: {
    label: string;
    segments: Array<{
        label: string;
        value: number;
    ...
    if (total <= 0 || visible.length === 0) {
        return (<div className="context-panel__source-bar-row">
        <span>{label}</span>
    ...
    return (<div className="context-panel__source-bar-row">
      ...
      <div className={`context-panel__source-bar${compactClass}`}>
        {visible.map((segment) => {
            ...
            return (<span className={`context-panel__source-bar-segment context-panel__source-bar-segment--${segment.tone}`} key={segment.tone} style={{ width: `${width}%` }} title={`${segment.label}: ${segment.value.toLocaleString()}`}/>);
        })}
      </div>
    </div>);
}
export { formatMetricTokens as formatMetricTokens } from "./context_panel_helpers";
export { formatCacheHitRate as formatCacheHitRate } from "./context_panel_helpers";
export { contextUsageRefreshKey as contextUsageRefreshKey } from "./context_panel_helpers";
export { cacheHitTone as cacheHitTone } from "./context_panel_helpers";
export { formatSharePercent as formatSharePercent } from "./context_panel_helpers";
export { contextCostDisplay as contextCostDisplay } from "./context_panel_helpers";
export { contextSessionCache as contextSessionCache } from "./context_panel_helpers";
export type { ContextPanelProps as ContextPanelProps } from "./context_panel_helpers";
export { fmtDuration as fmtDuration } from "./context_panel_helpers";
export { fmtUsageCacheRate as fmtUsageCacheRate } from "./context_panel_helpers";
export type { MetricTone as MetricTone } from "./context_panel_helpers";
export type { UsageAnalysisView as UsageAnalysisView } from "./context_panel_helpers";
export type { ContextWindowStatus as ContextWindowStatus } from "./context_panel_helpers";
export type { ContextBreakdown as ContextBreakdown } from "./context_panel_helpers";
export { nonNegativeTokenCount as nonNegativeTokenCount } from "./context_panel_helpers";
