...
import { RANGE_PRESETS, SOURCES, HEAT_WEEKS, MAX_TREND_DAYS, USAGE_STATS_TRANSLATIONS, UsageStatsTranslator, TOP_MODELS, OTHER_MODEL, OTHER_COLOR, MODEL_COLOR_MIX, MAX_TOOLTIP_OTHER_DETAILS, GroupedDaily, GroupedModel, localDay } from "./usage_stats_helpers";
import { StatCards } from "./usage_stats_cards";
export function UsageStatsPanel() {
    ...
    const loadHeat = useCallback(async () => {
        ...
        try {
            ...
            if (heatGenRef.current !== generation)
                return;
    ...
    const load = useCallback(async () => {
        ...
        const req: UsageStatsRequest = range === "custom"
    ...
    const dailyGrouped = useMemo<GroupedDaily[]>(() => {
        ...
        return (stats?.daily ?? []).map((d) => {
            ...
            if (other > 0)
                byModel[OTHER_MODEL] = other;
            return { ...d, byModel, otherByModel };
        });
    }, [stats]);
    const colorForModel = useCallback((model: string) => {
    ...
    return (<div className="usage-stats">
      ...
      <div className="usage-stats__toolbar">
        <div className="usage-stats__group" role="group" aria-label={t("settings.stats.range")}>
          {RANGE_PRESETS.map((r) => (<button key={r} type="button" className={`provider-add-segmented__item${range === r ? " provider-add-segmented__item--active" : ""}`} aria-pressed={range === r} onClick={() => setRange(r)}>
              {t(`settings.stats.rangePreset.${r}`)}
            </button>))}
          <button type="button" className={`provider-add-segmented__item${range === "custom" ? " provider-add-segmented__item--active" : ""}`} aria-pressed={range === "custom"} onClick={() => setRange("custom")}>
            {t("settings.stats.rangeCustom")}
          </button>
        </div>
        {range === "custom" && (<div className="usage-stats__custom">
            <input type="date" className="mem-input" value={customFrom} max={customTo || undefined} onChange={(e) => setCustomFrom(e.target.value)} aria-label={t("settings.stats.from")}/>
            <span className="usage-stats__custom-sep">–</span>
            <input type="date" className="mem-input" value={customTo} min={customFrom || undefined} max={localDay(0)} onChange={(e) => setCustomTo(e.target.value)} aria-label={t("settings.stats.to")}/>
          </div>)}
        <div className="usage-stats__group" role="group" aria-label={t("settings.stats.source")}>
          {SOURCES.map((s) => (<button key={s} type="button" className={`provider-add-segmented__item${source === s ? " provider-add-segmented__item--active" : ""}`} aria-pressed={source === s} onClick={() => setSource(s)}>
      ...
      {!loading && stats && (<>
          ...
          {stats.to && (<div className="usage-stats__foot">
              {t("settings.stats.asOf")} {stats.to}
            </div>)}
        </>)}
      {!loading && !error && stats && stats.tokens === 0 && (<div className="usage-stats__empty">{t("settings.stats.empty")}</div>)}
    </div>);
}
...
function Heatmap({ daily, from, to, t }: {
    ...
    const [geom, setGeom] = useState<{
        size: number;
        cols: number;
    }>({ size: HEAT_BASE, cols: HEAT_WEEKS });
    useEffect(() => {
    ...
    return (<section className="usage-stats__section">
      ...
      <div className="usage-stats__heatmap-wrap" ref={wrapRef}>
        <svg className="usage-stats__heatmap" width={weeks * (geom.size + HEAT_GAP) + HEAT_GAP} height={rows * (geom.size + HEAT_GAP) + HEAT_GAP} role="img" aria-label={t("settings.stats.heatmap")}>
          {days.map((day, i) => {
            ...
            return (<rect key={day} className={`usage-stats__heat-cell usage-stats__heat-cell--${level}`} x={x} y={y} width={geom.size} height={geom.size} rx={Math.max(1.5, geom.size * 0.2)} onMouseEnter={(e) => {
        ...
        {tip && (<div className="usage-stats__tip usage-stats__tip--chart" style={{ transform: `translate(${tipX}px, ${tipY}px) translate(-50%, ${tipAbove ? "-100%" : "0"})` }}>
...
function DailyTrend({ daily, modelOrder, t, colorForModel }: {
    daily: GroupedDaily[];
    modelOrder: string[];
    t: UsageStatsTranslator;
    colorForModel: (m: string) => string;
}) {
    ...
    const trendPointByDay = new Map<string, {
        x: number;
        y: number;
    }>();
    visible.forEach((d, i) => {
    ...
    if (tip && !tipAbove && tipY + tipH > cBottom - 8)
        tipY = tip.top - 10;
    return (<section className="usage-stats__section">
      <div className="usage-stats__section-head">
      ...
      <div className="usage-stats__chart-wrap" ref={wrapRef}>
        <svg className="usage-stats__chart" width="100%" height={H} viewBox={`0 0 ${plotWUsed} ${H}`} onMouseLeave={() => { setTip(null); setHover(null); }}>
          ...
          {visible.map((d, i) => {
            ...
            return (<g key={d.day}>
                {bars.map((b) => {
                    ...
                    return (<rect key={`${d.day}-${b.model}`} className={`usage-stats__bar${b.dimmed ? " usage-stats__bar--dim" : ""}`} x={b.x} y={b.y} width={barW} height={b.h} fill={b.color} style={hovered ? { transform: `scaleX(${(barW + 3) / barW})` } : undefined}/>);
                })}
                ...
                <rect className="usage-stats__bar-hit" x={x} y={padT} width={barW} height={plotH} onMouseEnter={(e) => {
                    const r = e.currentTarget.getBoundingClientRect();
                ...
                {(i % Math.max(1, Math.floor(n / 8)) === 0 || i === n - 1) && (<text className="usage-stats__axis" x={padL + barHalf + i * step} y={H - 8} textAnchor="middle">{shortDay(d.day)}</text>)}
              </g>);
        })}
          ...
          {trendTipPt && (<circle className="usage-stats__trend-dot" cx={trendTipPt.x} cy={trendTipPt.y} r={4}/>)}
        ...
        {tip && (<div className="usage-stats__tip usage-stats__tip--chart usage-stats__tip--screen" style={{ transform: `translate(${tipX}px, ${tipY}px) translate(-50%, ${tipAbove ? "-100%" : "0"})` }}>
            <div className="usage-stats__tip-title">{tip.day}</div>
            <div>{t("settings.stats.total")}: {formatTokens(tip.total)}</div>
            {modelOrder.filter((m) => tip.byModel[m] !== undefined).map((m) => (<div key={m} className="usage-stats__tip-row"><i className="usage-stats__legend-swatch" style={{ background: colorForModel(m) }}/>{m === OTHER_MODEL ? t("settings.stats.other") : m}: {formatTokens(tip.byModel[m])}</div>))}
            {tipOtherVisible.map(([m, v]) => (<div key={m} className="usage-stats__tip-row usage-stats__tip-row--other"><i className="usage-stats__legend-swatch" style={{ background: OTHER_COLOR }}/>{m}: {formatTokens(v)}</div>))}
        ...
        <div className="usage-stats__legend">
          {legendModels.map((model) => (<span key={model} className="usage-stats__legend-item" onMouseEnter={() => setHover(model)} onMouseLeave={() => setHover(null)}>
...
function ModelUsage({ models, t, colorForModel }: {
    models: GroupedModel[];
    t: UsageStatsTranslator;
    colorForModel: (m: string) => string;
}) {
      ...
      <div className="usage-stats__models">
        <div className="usage-stats__donut-wrap" ref={donutRef}>
          <svg className="usage-stats__donut" width={CX * 2} height={CX * 2} viewBox={`0 0 ${CX * 2} ${CX * 2}`} role="img" aria-label={t("settings.stats.modelUsage")}>
            ...
            const el = (<circle key={m.model} className={`usage-stats__donut-seg${hover !== null && !active ? " usage-stats__donut-seg--dim" : ""}`} cx={CX} cy={CX} r={R} fill="none" stroke={colorForModel(m.model)} strokeDasharray={`${dash} ${CIRC - dash}`} strokeDashoffset={-offset} transform={`rotate(-90 ${CX} ${CX})`} style={{ strokeWidth: active ? SW + 5 : SW, transition: "stroke-width 0.12s ease" }} onMouseEnter={(e) => {
          ...
          {tip && (<div className="usage-stats__tip usage-stats__tip--chart" style={{ transform: `translate(${tipX}px, ${tipY}px) translate(-50%, ${tipAbove ? "-100%" : "0"})` }}>
              ...
              {tipItems.length > 0 && (<div className="usage-stats__tip-breakdown">
                  {tipItemsVisible.map((it) => (<div key={it.model} className="usage-stats__tip-row usage-stats__tip-row--other"><i className="usage-stats__legend-swatch" style={{ background: OTHER_COLOR }}/>{it.model}: {formatTokens(it.tokens)}</div>))}
          ...
          {models.map((m) => {
            ...
            const rowContent = (<>
                ...
                <span className="usage-stats__model-name">
                  {isOther && (<span className="usage-stats__model-toggle" aria-hidden="true">
            ...
            if (isOther) {
                return (<li key={m.model} className="usage-stats__model-item">
                  <button type="button" className="usage-stats__model-row usage-stats__model-row--expandable" onMouseEnter={() => setHover(m.model)} onMouseLeave={() => setHover(null)} onClick={() => setExpandedOther((open) => !open)} aria-expanded={expandedOther}>
                    {rowContent}
                  </button>
                </li>);
            }
            return (<li key={m.model} className="usage-stats__model-row" onMouseEnter={() => setHover(m.model)} onMouseLeave={() => setHover(null)}>
                {rowContent}
              </li>);
        })}
          {other?.items && other.items.length > 0 && (<li className={`usage-stats__model-other-wrap${expandedOther ? " usage-stats__model-other-wrap--open" : ""}`}>
              <ul className="usage-stats__model-other-list">
                {other.items.map((it) => (<li key={it.model} className="usage-stats__model-row usage-stats__model-row--sub">
...
export { RANGE_PRESETS as RANGE_PRESETS } from "./usage_stats_helpers";
export { SOURCES as SOURCES } from "./usage_stats_helpers";
export { HEAT_WEEKS as HEAT_WEEKS } from "./usage_stats_helpers";
export { MAX_TREND_DAYS as MAX_TREND_DAYS } from "./usage_stats_helpers";
export { USAGE_STATS_TRANSLATIONS as USAGE_STATS_TRANSLATIONS } from "./usage_stats_helpers";
export type { UsageStatsTranslator as UsageStatsTranslator } from "./usage_stats_helpers";
export { TOP_MODELS as TOP_MODELS } from "./usage_stats_helpers";
export { OTHER_MODEL as OTHER_MODEL } from "./usage_stats_helpers";
export { OTHER_COLOR as OTHER_COLOR } from "./usage_stats_helpers";
export { MODEL_COLOR_MIX as MODEL_COLOR_MIX } from "./usage_stats_helpers";
export { MAX_TOOLTIP_OTHER_DETAILS as MAX_TOOLTIP_OTHER_DETAILS } from "./usage_stats_helpers";
export type { GroupedDaily as GroupedDaily } from "./usage_stats_helpers";
export type { GroupedModel as GroupedModel } from "./usage_stats_helpers";
export { localDay as localDay } from "./usage_stats_helpers";
export { StatCards as StatCards } from "./usage_stats_cards";
